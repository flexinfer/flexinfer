...
def patch_moe_wna16_rocm_reference_fallback(vllm_root: pathlib.Path) -> bool:
    ...
    new_block = (
        ...
        "                device=x.device,\n"
        "            )\n"
        "            # Hoist topk_ids GPU→CPU sync out of the per-slot inner loop.\n"
        "            # Each .item() call on a CUDA/HIP tensor forces a full pipeline\n"
        "            # flush; py-spy on cblevins-5930k showed ~50% of decode samples in\n"
        "            # this patch path. With top_k=8 across 30 MoE layers that's 240\n"
        "            # syncs per generated token. Single .tolist() does one sync for\n"
        "            # the whole tensor; the loop then iterates as Python ints (no GPU\n"
        "            # traffic until the matmul). Same outputs.\n"
