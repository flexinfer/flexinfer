...
def patch_moe_wna16_rocm_reference_fallback(vllm_root: pathlib.Path) -> bool:
    ...
    new_block = (
        ...
        "                # B1: LRU on cache hit — move key to end so it's last-evicted.\n"
        "                # R3: cap raised from 16 -> 24 entries. Instrumentation\n"
        "                # showed top16_share=50% / top32_share=70% expert reuse\n"
        "                # in steady state; cap=24 captures ~60% with ~0.7 GB of\n"
        "                # added VRAM growth, fitting alongside gpu_mem_util=0.98.\n"
        "                # cap=32 would gain ~3% more but failed the vLLM startup\n"
        "                # KV-block budget check on this host.\n"
        "                try:\n"
        ...
        "                return _cache_put(key, _w.transpose(0, 1).contiguous())\n"
        "\n"
        "            # B3a (batched dequant across top_k experts) was attempted in\n"
        "            # an earlier overlay and reverted: peak memory of (N, I*2, H)\n"
        "            # FP16 intermediate (~120 MiB for top_k=8) does not fit\n"
        "            # alongside gpu_memory_utilization=0.98. Per-expert loop below\n"
        "            # stays. If a future change relaxes the memory budget, a\n"
        "            # batched dequant helper can be added here that gathers cache\n"
        "            # misses, dequants in a single pass, and populates the cache.\n"
        "\n"
        "            _result = torch.zeros(\n"
        ...
        "                device=x.device,\n"
        "            )\n"
        "            # Vectorize the per-slot inner loop: collapse top_k separate\n"
        "            # matmul kernel launches into 2 torch.bmm calls per token. The\n"
        "            # original loop fired 8 matmuls into w13 + 8 into w2 = 16 small\n"
        "            # kernel launches per layer per token, × 30 layers × N tokens.\n"
        "            # py-spy after a prior topk_ids.tolist() hoist showed compute\n"
        "            # lines (`_get_w13`, `_get_w2`, the matmuls) as the new\n"
        "            # bottleneck — small matmul kernel launch overhead × the X99\n"
        "            # platform's higher PCIe 3.0 launch latency.\n"
        "            #\n"
        "            # Math equivalence:\n"
        "            #   gate_up = x @ W13[e] is LINEAR; stacking W13[e_0..e_K-1]\n"
        "            #     and bmm-ing with replicated x produces numerically\n"
        "            #     identical results modulo FP16 accumulation order.\n"
        "            #   apply_moe_activation is a per-row nonlinearity, so calling\n"
        "            #     it on (top_k, intermediate*2) gives the same output as\n"
        "            #     top_k separate (1, intermediate*2) calls.\n"
        "            #   Final sum_s router_weight[s] * expert_out[s] is associative.\n"
        "            #   apply_router_weight_on_input controls whether router_w is\n"
        "            #     multiplied INTO the input (before activation) vs INTO the\n"
        "            #     output (after activation). These are NOT equivalent\n"
        "            #     because activation is nonlinear; preserve the branch.\n"
        "            #\n"
        "            # Negative expert ids (sentinel for 'skip this slot') are\n"
        "            # handled by zeroing the router weight for those slots and\n"
        "            # substituting expert 0 in the gather. Cheap mask; rare.\n"
        "            _topk_ids_cpu = topk_ids.tolist()\n"
        "            _topk_weights_fp16 = topk_weights.to(torch.float16)\n"
        "            _topk_n = topk_ids.shape[1]\n"
        "            for _tok in range(x.shape[0]):\n"
        "                _tok_x = x[_tok : _tok + 1].to(torch.float16)\n"
        "                _tok_experts = _topk_ids_cpu[_tok]\n"
        "                _safe_eids = [eid if eid >= 0 else 0 for eid in _tok_experts]\n"
        "                _valid_mask_py = [1.0 if eid >= 0 else 0.0 for eid in _tok_experts]\n"
        "                _w13_batch = torch.stack(\n"
        "                    [_get_w13(eid) for eid in _safe_eids], dim=0\n"
        "                )\n"
        "                _w2_batch = torch.stack(\n"
        "                    [_get_w2(eid) for eid in _safe_eids], dim=0\n"
        "                )\n"
        "                _tok_router_w = _topk_weights_fp16[_tok] * torch.tensor(\n"
        "                    _valid_mask_py, dtype=torch.float16, device=x.device\n"
        "                )\n"
        "                if layer.apply_router_weight_on_input:\n"
        "                    _expert_in_batch = (\n"
        "                        _tok_x.unsqueeze(0).expand(_topk_n, -1, -1)\n"
        "                        * _tok_router_w.view(-1, 1, 1)\n"
        "                    )\n"
        "                else:\n"
        "                    _expert_in_batch = _tok_x.unsqueeze(0).expand(\n"
        "                        _topk_n, -1, -1\n"
        "                    )\n"
        "                # (top_k, 1, H) @ (top_k, H, I*2) -> (top_k, 1, I*2)\n"
        "                _gate_up_batch = torch.bmm(_expert_in_batch, _w13_batch)\n"
        "                _intermediate_size = _w13_batch.shape[2] // 2\n"
        "                _gate_up_2d = _gate_up_batch.view(_topk_n, -1)\n"
        "                _hidden_2d = torch.empty(\n"
        "                    (_topk_n, _intermediate_size),\n"
        "                    dtype=torch.float16,\n"
        "                    device=x.device,\n"
        "                )\n"
        "                apply_moe_activation(\n"
        "                    layer.activation, _hidden_2d, _gate_up_2d\n"
        "                )\n"
        "                _hidden_batch = _hidden_2d.unsqueeze(1)\n"
        "                # (top_k, 1, I) @ (top_k, I, H) -> (top_k, 1, H)\n"
        "                _expert_out_batch = torch.bmm(_hidden_batch, _w2_batch)\n"
        "                if not layer.apply_router_weight_on_input:\n"
        "                    _expert_out_batch = (\n"
        "                        _expert_out_batch * _tok_router_w.view(-1, 1, 1)\n"
        "                    )\n"
        "                _tok_out = _expert_out_batch.sum(dim=0)\n"
        "                _result[_tok : _tok + 1] = _tok_out.to(_result.dtype)\n"
        "        else:\n"
        "            _result = fused_experts(\n"
        "                x,\n"
        "                layer.w13_qweight,\n"
...
def patch_moe_wna16_activation_forwarding(vllm_root: pathlib.Path) -> bool:
    ...
    new_call = (
        "        # GEMMA4_MOE_ACTIVATION_FORWARD_PATCH: Forward activation\n"
        "        # B2: NaN/Inf .item() debug guards removed — were 4 GPU->CPU syncs per\n"
        "        # layer per token (~16,800 syncs/request). See profile note\n"
        "        # research-26b-5930k-profile-2026-05-14.md.\n"
        "        _result = fused_experts(\n"
        "            x,\n"
        "            layer.w13_qweight,\n"
        "            layer.w2_qweight,\n"
    ...
    if match:
        indent = match.group(1)
        replacement = (
            f"{indent}# GEMMA4_MOE_ACTIVATION_FORWARD_PATCH (B2: NaN debug removed)\n"
