...
**Status**: Slice 1 v2 INCONCLUSIVE (cache strategy mismatch, NOT a Whisper/ROCm failure). Slices 2 + 3b shipped 2026-05-18 via [!423](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/423). Slice 1 v1 attempted 2026-05-19 via [!429](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/429) — INCONCLUSIVE (structural deadlock), cleaned up via !430. Slice 1-bis (`gpu.forcePromotion` field + `chooseSharedGroupLeader` short-circuit) shipped 2026-05-19 via [!433](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/433) (merge commit `9f251180`). Slice 1 v2 shipped 2026-05-19 via [!436](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/436) (merge commit `80fd3462`); on-cluster validation **verified the force-promotion chooser path + preemption end-to-end** but the kill-test pod crashed before vLLM loaded Whisper weights because the cache PVC files never landed at the runtime's hostPath mount. Full v2 evidence in `.loom/asr-diarization-kill-test-v2-inconclusive-2026-05-19.md`. Next: tear-down MR + cache-layout rewrite for v3.
...
**Status**: v1 ATTEMPTED 2026-05-19, INCONCLUSIVE — structural deadlock in `chooseSharedGroupLeader` (Ready warm-primary 26B always beat the new claimant). Slice 1-bis (`gpu.forcePromotion` field) shipped via [!433](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/433) (merge commit `9f251180`) on 2026-05-19. **v2 SHIPPED 2026-05-19 via [!436](https://gitlab.flexinfer.ai/services/flexinfer/-/merge_requests/436) (merge commit `80fd3462`), on-cluster outcome INCONCLUSIVE (different reason)**: force-promotion chooser + preemption verified end-to-end, but vLLM crashed before loading Whisper weights due to a cache strategy mismatch — the kill-test Model CR declared only a cache PVC and no main model PVC, so the `cache-copy` job never fired and the runtime DaemonSet's hostPath mount stayed empty. vLLM was invoked with `--model /models/whisper-kill-test-v2`, hit transformers' `cached_file` fallback, then `hf_hub_download` rejected the absolute path as an invalid repo ID. Full v2 evidence: `.loom/asr-diarization-kill-test-v2-inconclusive-2026-05-19.md`. Side-finding: the controller startup-sweep at `controllers/model_controller.go:504-541` skips empty-Phase Models, so the kill-test sat unreconciled for 19 min after a controller image rollout until a manual annotation forced enqueue — same root cause as the v1 "8-hour resourceVersion stall" hypothesis (line 146 of the v1 doc). **Next**: tear-down MR (remove `deploy/models/whisper-kill-test-v2.yaml`), then v3 with proper main+cache PVC layout (mirroring `gemma4-26b-a4b-gptq`) so the runtime actually sees the model files.
...
14. **REVISED 2026-05-19 (post-investigation)**: original framing said "the controller did not reconcile the kill-test Model CR" and hypothesized that the Model reconciler gates on `serviceLabels` or `litellm.enabled`. **Static-code investigation falsifies that hypothesis**. The real root cause is structural — the shared-GPU swap logic has a chicken-and-egg deadlock when a new claimant joins a group that already has a Ready warm-primary member.

    **Code evidence**:
    - `controllers/model_controller.go:488-498` (`SetupWithManager`) uses bare `For(&aiv1alpha2.Model{}).Owns(...)` with no predicate filter. Manager has no namespace restriction (`cmd/flexinfer-manager/main.go:91-97`). All `ai.flexinfer/v1alpha2` Models in any namespace flow into `Reconcile`. No `serviceLabels` or `litellm.enabled` gating exists in any controller path.
    - `controllers/model_shared_gpu.go:50-185` (`chooseSharedGroupLeader`) iterates the group and returns the first matching candidate in priority order: `activeLoadingLeader` → `demandedLeader + readyLeader` → `readyLeader` alone → `recentLeader/warmPrimaryLeader` → `runnableFallbackLeader` → `fallbackLeader`. **The kill-test (Phase ∈ {empty, Pending}) cannot be a `readyLeader`. The 26B (Phase=Ready) IS a `readyLeader` and wins at priority step 3, BEFORE the function ever reaches `fallbackLeader` where the kill-test could win on raw priority 400 > 350.**
    - `controllers/model_helpers.go:50-63` (`desiredReplicasForContext`) returns `0` for any shared-group Model whose `Status.SharedGroup.State != "Active"`. After `chooseSharedGroupLeader` picks the 26B, the kill-test is set to `State: "Queued"`. desiredReplicas=0. **No Deployment is ever created.** Without a Deployment the Model never becomes Ready. Without Ready it can never win `readyLeader` selection. Deadlock by construction.
    - `controllers/model_shared_gpu.go:154-164` (demand-based swap escape hatch) requires `demandedLeader.Spec.GetPriority() > readyLeader.Spec.GetPriority()` *and* a recent `LastActiveTime` (within `sharedDemandWindow = 2 * time.Minute`). The proxy writes `LastActiveTime` only on routed requests, which requires `serviceLabels`. The kill-test deliberately omitted `serviceLabels`. So `demandedLeader == nil` and this escape hatch never fires.

    **Implication for Slice 3a**: the production Whisper Model CR as currently specced (`priority: 100`, well below 26B's `350`) **cannot win leadership**. The demand-based-swap condition is `demanded.Priority > ready.Priority`, OR `ready idle && demanded.Priority >= ready.Priority`. With priority 100 < 350, the first branch never fires. The second branch requires the warm 26B to go idle for >2 min — but the 26B is `warmPolicy: primary` so `desiredReplicasForContext` keeps it at minReplicas=1 (`model_helpers.go:69-72`). Slice 3a's Whisper would join `7900xtx-textgen` and sit at `State: Queued, replicas: 0` forever, regardless of `serviceLabels` presence.

    **Mechanisms that DO work**:
    - Set `priority` > 350 AND give the Model `serviceLabels` so the proxy can route to it. First request to `/v1/audio/transcriptions` writes `LastActiveTime`, fires `demandedLeader` swap (priority > 350 satisfies the strict-greater branch), 26B yields. But this couples the kill-test to a live request path, which the original plan deliberately avoided.
    - **Or** add a controller mechanism that does not exist today: a `flexinfer.ai/pause: "true"` annotation on the 26B (referenced in the original plan as if it existed; it does NOT — `grep -r 'flexinfer.ai/pause' controllers/ api/` returns nothing), OR a `gpu.forcePromotion: true` spec field on the new claimant, OR a `gpu.warmupAllowed: true` field that exempts a new claimant from the `chooseSharedGroupLeader` Ready-first preference so it gets a Deployment.
    - **Or** run the kill-test on a node that does NOT host a warm-primary 26B. cblevins-5930k also hosts a 26B; the only gfx1100 node without a 26B is none in the current fleet. cblevins-radeonvii (gfx906) is unsuitable per `audioTranscription: unsupported` (the !423 schema). Practical answer: there is no gfx1100 node without a warm-primary at the moment.

    **Separate operational mystery (not explained by the above)**: even granting that the kill-test would end up `State: Queued, replicas: 0`, that state requires `Reconcile` to fire at least once and call `handleSharedGPU` + `Status().Update`. But the evidence shows `resourceVersion: 637002597` unchanged for 8+ hours between snapshots — the controller never wrote to this Model at all. `Reconcile`'s first action is to add a finalizer via `r.Update` (`model_controller.go:130-138`) which would bump resourceVersion. That this didn't happen suggests **the controller's informer cache never delivered the watch event** for this Model. Possible causes: leader-election flap during the gfx906 commit rollouts (`5b134f28`, `9af52fc9`, `864a30ed`); stale informer; pod restart that lost the event. This is a SECOND, separate finding that warrants its own diagnostic in a follow-up — it can't be reproduced from static analysis alone.

    **Investigation required before re-attempting Slice 1**: implement one of the "Mechanisms that DO work" above (recommend the `gpu.forcePromotion` spec field — cleanest, surgical, also useful for production canary rollouts). Then re-attempt Slice 1 with the new field set. Separately, capture controller logs and pod restart history from the kill-test window (04:01Z – 12:40Z 2026-05-19) to determine whether the no-reconcile was a leader-election event or something else.

    **Slice 3a re-design**: the current spec (`priority: 100`, `serverless.minReplicas: 0`) will not work. Choose one of:
    - (a) `gpu.forcePromotion: true` (requires the new spec field — code change in Slice 1 retry)
    - (b) `priority: 400` (above 26B), accepting that every transcribe request causes a full 26B unload + 1.5 GiB Whisper load + transcribe + Whisper teardown + 26B reload (`swapCooldown` keeps this bounded but it's expensive). Only viable if ICC's transcribe rate is <1/hour.
    - (c) Move the warm 26B off cblevins-7900xtx (impractical — both gfx1100 nodes host warm primaries).
    - Document the chosen approach before applying Slice 3a's Model CR.
...
|---|---|---|---|---|
| 1 | Kill-test passes against pinned vLLM image | flexinfer maintainer | 🔄 v2 INCONCLUSIVE (cache strategy mismatch, NOT Whisper/ROCm); chooser + preemption verified live | `.loom/asr-diarization-kill-test-inconclusive-2026-05-19.md` (v1); `.loom/asr-diarization-kill-test-v2-inconclusive-2026-05-19.md` (v2); next step: tear-down + v3 with main PVC |
| 1-side | Controller startup-sweep enqueues empty-Phase Models | maintainer | ❌ BUG confirmed (v1 + v2 both bit by it); workaround = annotation bump | `model_controller.go:504-541` filter on `Phase ∈ {Loading, Pending}` excludes new CRs |
