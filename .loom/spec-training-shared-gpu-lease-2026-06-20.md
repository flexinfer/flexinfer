...
2. **Slice 2 — `GPULease` carrier + election integration** — CRD/types + `chooseSharedGroupLeader` honors active leases (park-and-hold) + unit tests (election returns parked-all while leased; re-promotes on release; warm-primary not stranded; cooldown respected).
