...
5. **Slice 5 — Hardening** — **preempt-policy threshold ✅ (MR !691)**: `GPULease.spec.priority` (fed from `ModelCache.spec.finetune.gpuLease.priority`) gates park-and-hold via `leaseFreesCard` — a lease frees the card only when it strictly outranks every serving member; a member at/above the threshold keeps the card and the training Job waits. Nil priority = unconditional park-and-hold (backward-compatible default; the live-proven slice-4 path). **Observability ✅ (MR !692)**: a lease park now sets a distinct `GPULeaseHeld` ModelReady condition reason (vs generic `Preempted`) + a "Card held by training" message, derived from the `gpu-lease/<owner>` `SharedGroup.PreemptedBy` attribution and applied consistently across the shared-GPU election, runtime-unload, and deployment status paths — so dashboards/proxy/operators don't read a parked lane as an outage (the whisper-phantom lesson). Still open: SwapCooldown interplay, multi-GPU cards (7900xtx 2-slot: lease one slot vs both), training-vs-training queueing, a Grafana/alert surface consuming `GPULeaseHeld` + a runbook.
