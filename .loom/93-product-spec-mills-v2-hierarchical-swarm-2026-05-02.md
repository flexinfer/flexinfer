# Product Spec: Loom Mills v2 — Hierarchical Swarm + Adversarial Audit + Cross-Repo Federation

**Date**: 2026-05-02
**Research**: `.loom/92-research-mills-v2-hierarchical-swarm-2026-05-02.md`
**Implementation Plan**: `.loom/94-implementation-plan-mills-v2-hierarchical-swarm-2026-05-02.md`

## Goal

Promote Loom Mills from a flat two-tier system (Council → Pipeline) to a true hierarchical swarm:
...
8. **Mobile Mills parity** (deferred from v1.1 #1): one screen.

V1.1 deferral #2 (per-stage scoped credentials) is **not** in v2 — kept as v2.1 follow-up.

## Non-goals

- New runtime substrate. Same `loom-mills-operator` deployment, same canonical SQLite, same MentatLab pipeline engine.
...
| V2-D2 | Squad memory storage | **New tables in canonical store** (not agent-context). Mills remains self-contained. |
...
| V2-D8 | New migrations | **`pkg/mills/store/migrations/002_v2.sql`**, append-only, reviewable. |
...
v2:   Council (debate) ──► Squad Router ──► Squad{N} ──► Pipeline (recursive ≤2)
              ...
              └─► Adaptive Policy Engine ──► policy diff proposals
                                              (.loom/mills/policy_proposals/)
```

Concretely, every v2 piece is implemented as a new Go package under `pkg/mills/` plus migrations and HUD additions; nothing about the v1 reconciler, eval, or pipeline runner changes its public contract.

## Persistence layer (v2 deltas)

New SQLite migration `pkg/mills/store/migrations/002_v2.sql` adds:
...
-- Adaptive policy proposals (.loom/mills/policy_proposals/<date>.md is the human-facing copy)
...
DAO additions live in `pkg/mills/store/dao_squad.go`, `dao_audit.go`, `dao_crossrepo.go`, `dao_debate.go`, `dao_policy_proposal.go`. Each is a new file; no modifications to existing DAOs except the new columns on `pipeline_runs` accessed via existing `dao_pipeline.go` queries (verify in implementation plan slice 1).
...
`platform/gitops/k3s/mills/squads/_default.yaml` (used when no squad matches; identical to v1 behavior):
...
Audit prompts (in `pkg/mills/audit/prompts/`):
...
Repo registry `platform/gitops/k3s/mills/repos.yaml`:
...
- `policy.council.debate.enabled.manual`: true (operator can `loom mills council run --debate`)
...
New MCP tool exposed by `loom-mills-operator`:

- `mcp-mills::pipeline_subrun_create(parent_run_id, slice_spec, depth)` — creates a child `pipeline_runs` row with `parent_run_id` set; returns child run id. Caller (a worker pod with admin token) gets back a per-subrun budget.

Enforcement (in `pkg/mills/pipeline/recursion.go`):
...
KPI: `loom_mills_pipeline_recursion_depth_histogram` (p95 expected between 1 and 2).
...
Emits:
- A markdown file `.loom/mills/policy_proposals/<YYYY-MM-DD>.md` summarizing proposals with citations to KPI rows.
...
v2.0: All proposals are `state=pending` until human edit applies the policy. Human applies by editing `platform/gitops/k3s/mills/configmap-policy.yaml` (and `squads/*.yaml` for ensemble changes); operator hot-reloads (existing `pkg/mills/policy.go` fsnotify path).
...
Surface in HUD `Backlog` panel and `loom mills backlog show <id>`:
...
Implementation: `pkg/mills/budget/estimator.go` consults:
...
One screen in iOS companion app (`apps/loom-companion-ios/`). Renders three KPI cards and a list of in-flight pipelines. Read-only in v2.0; "Pause Mills" button gated to v2.1.

API: existing operator REST endpoints already cover the data. Mobile view consumes:
- `GET /api/mills/status`
- `GET /api/mills/pipeline/runs?state=running`
- `GET /api/mills/audit/findings?since=24h`

## REST + MCP surface (v2 additions)

### REST

- `GET    /api/mills/squads` — list squads with config and 30-day outcomes
- `GET    /api/mills/squads/{name}` — full squad detail
- `GET    /api/mills/squads/{name}/memory` — paginated working memory
- `POST   /api/mills/squads/{name}/route-test` — admin: simulate routing for a backlog id and return chosen squad + confidence
- `GET    /api/mills/audit/findings?subject_kind=&subject_id=&since=` — filter
- `POST   /api/mills/audit/run` — admin: re-run audit on a subject id
- `GET    /api/mills/cross-repo/runs` — list with state filter
- `POST   /api/mills/cross-repo/runs/{id}/abort` — admin: abort + revert
- `GET    /api/mills/policy/proposals?state=pending|applied|rejected`
- `POST   /api/mills/policy/proposals/{id}/apply` — admin v2.0 manual apply (writes to ConfigMap; logs proposal as state=applied_human)
- `GET    /api/mills/cost-preview?backlog_id=` — pre-spawn estimate

### MCP (via `mcp-mills` server, served by the operator)

- `mills_squads_list`
- `mills_squad_memory_recall(squad, query, limit)` — semantic search over `squad_memory`
- `mills_audit_findings_list(subject_kind, subject_id)`
- `mills_pipeline_subrun_create(parent_run_id, slice_spec, depth)` — bounded-recursion entry point (admin-token only)
- `mills_cost_preview(backlog_id)`
- `mills_policy_proposals_list(state)`

## Policy file additions

```yaml
# v2 keys appended to platform/gitops/k3s/mills/configmap-policy.yaml
mills:
  enabled: true
  cross_repo:
    enabled: false
...
8. **Mobile parity**: iOS Mills screen renders three KPI cards + in-flight pipelines list end-to-end against the cluster operator (read-only).
...
## KPIs (HUD `Mills` view extensions)
...
New card on Overview: `Adaptive Policy Proposals` — pending count, link to `.loom/mills/policy_proposals/`.
...
- `pkg/mills/store/types.go`, `pkg/mills/store/migrate.go` (current migration pattern)
- `pkg/mills/policy.go` (fsnotify hot-reload pattern)
- `pkg/mills/pipeline/integrator.go` (existing fan-out pattern)
- `pkg/mills/pipeline/dispatcher.go` (worker dispatch pattern)
- `internal/hud/frontend/src/lib/components/Mills/` (existing 4 panels)
