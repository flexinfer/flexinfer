...
| `qwen3-1p7b-vllm-radeonvii` vLLM canary prerequisites (2026-05-17) | 4096 | `gfx906/radeonvii` | `vllm` | `experimental` | Persistent runtime `registry.harbor.lan/flexinfer/runtime@sha256:cbe1157c2fb6a24fc67e901bec92a72bbf16498a86ad1a064ce9bf4db1f2ddf4`; standalone vLLM image `registry.harbor.lan/flexinfer/vllm:rocm-gfx906@sha256:beadf394fc81c031799799f5d965664e419e5f3ffb4c5873a9d7677f0e1e06b8` | `HF://Qwen/Qwen3-1.7B`; Local cache at `/models/flexinfer-system/qwen3-1p7b-vllm-radeonvii` | RALPH fixed the canary prerequisites: resource name is DNS-label-safe (`qwen3-1p7b-vllm-radeonvii`), cache strategy is `Local`, priority is `130` so explicit canary traffic can preempt the CPU fallback, and public aliases remain canary-scoped. Local validation passed: `kubectl kustomize deploy/models` and server-side dry-run. Live test staged the HF model into local cache, then a proxy canary demand moved the model to shared-group `Active` and preempted the fallback. MR !412 changed the controller to use runtime pods with a PodIP for load/unload even when Kubernetes Ready=false. This follow-up adds `runtime.bundledBackends` so gfx906 `backend: vllm` skips direct runtime load and uses the standalone profile backend image instead. The 2026-05-18 follow-up pins the GPUProfile/Helm gfx906 vLLM image to this digest and adds the `radeonvii-gfx906-vllm-canary` imagePrewarm hold profile so the next smoke starts with the standalone image cached and visible. | No HTTP 200 vLLM smoke yet. The latest blocker is a profile/packaging mismatch: the persistent `runtime:rocm-gfx906` image does not bundle the `vllm` Python module, while the standalone `registry.harbor.lan/flexinfer/vllm:rocm-gfx906` image is the intended canary image. Before the next smoke, verify the imagePrewarm DaemonSet is Ready and Radeon VII remains `DiskPressure=False`; then activate the canary. Before the bundled-backend contract, after cache staging and preemption, the Model stayed `Idle/Active` because the controller logged `Runtime pod exists but not ready, waiting for it to start`; the persistent runtime API was live, but `/readyz` hung after the aborted load, so the controller never issued the next load request. During hot apply, the old dotted-name model also caused temporary alias-conflict warnings; the rename/prune removes that conflict after merge. Runtime was recycled and `qwen3-1p7b-tools-radeonvii` recovered to `Ready` with a direct chat smoke returning `Blue`. | `kubectl -n flexinfer-system get ds -l flexinfer.ai/prewarm-profile=radeonvii-gfx906-vllm-canary` then `kubectl -n flexinfer-system run smoke --rm -i --restart=Never --image=curlimages/curl:8.11.1 -- curl -sS -m 120 -X POST http://flexinfer-proxy.flexinfer-system.svc.cluster.local/model/qwen3-1p7b-vllm-radeonvii/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"qwen3-1.7b-vllm","messages":[{"role":"user","content":"The color of the sky is"}],"max_tokens":16,"temperature":0}'` | Delete `Model/qwen3-1p7b-vllm-radeonvii`, restore `qwen3-1p7b-tools-radeonvii` `minReplicas: 1`, and recycle `flexinfer-runtime-gfx906` if `/readyz` hangs. | `.loom/ralph-gfx906-vllm-smoke-2026-05-17.md`; `deploy/models/qwen3-1p7b-vllm-radeonvii.yaml`; `controllers/model_controller.go`; `controllers/runtime_controller.go`; `pkg/runtime/payload.go`; `deploy/gpuprofiles/gfx906.yaml`; `deploy/system/values-k3s.yaml`; evidence dir `/tmp/flexinfer-gfx906-vllm-fixed-smoke-20260517T193341Z` | `block` |
