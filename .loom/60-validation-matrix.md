...
| `qwen3-1p7b-tools-radeonvii` GGUF tool-router | 8192 | `gfx906/radeonvii` | `llamacpp` | `experimental` | Persistent runtime `registry.harbor.lan/flexinfer/runtime@sha256:b36bb0ab008efa3d8a127cdf7cd9813c8ad88ddf7d62d8736bc6ad0976fe20f0`; standalone profile image `registry.harbor.lan/library/llamacpp:rocm-gfx906-patched-v3` digest TBD | `HF://rippertnt/Qwen3-1.7B-Q4_K_M-GGUF` / `qwen3-1.7b-q4_k_m.gguf` | Cache prefetched and ready on 2026-05-16. MR !394 fixed runtime-manager fallback from stale absolute backend paths to `PATH`; MR !395 added the missing live `runtime:rocm-gfx906` publish job. Master pipeline 9917 published `runtime:rocm-gfx906-a21153d4` at the prior digest and proved command fallback (`/opt/src/.../llama-server` resolved to `/opt/llamacpp/bin/llama-server`). MR !397 separated the flexinfer-runtime API port (`8080`) from the runtime-managed backend port (`8000`); master pipeline 9946 published `runtime:rocm-gfx906-6c92809b` at this digest. | Port fix rollout passed its contract: live args showed `--port 8000`. The next blocker is cache/path shape: this Model used `SharedPVC`, but runtime DaemonSets only see node-local `/models/flexinfer-system/...` caches; runtime payloads also need to append `config.ggufFile` so llama.cpp receives the actual GGUF file path. Keep `tool-router` and `qwen3-1.7b` aliases only; do not make this the default chat route unless coherence and latency pass. | After Local-cache + GGUF-path patch rollout: `kubectl -n flexinfer-system port-forward svc/flexinfer-proxy 8080:80`; `curl -sS http://127.0.0.1:8080/model/qwen3-1p7b-tools-radeonvii/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"qwen3-1.7b-tools","messages":[{"role":"user","content":"Reply with exactly one word: blue"}],"max_tokens":8,"temperature":0}'` | `registry.harbor.lan/flexinfer/runtime@sha256:a536d25f8b0f154e16c080ce852da6f1430783bedcb53e7098cee3a9baa264c5` | RG-4 / `.loom/real-hardware-platform-improvements-plan.md` Slice 4; MR !394; MR !395; MR !397; pipelines 9917 and 9946; 2026-05-16 live failure evidence below | `pending` |
