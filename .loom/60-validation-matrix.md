...
| `qwen3-1p7b-tools-radeonvii` GGUF tool-router | 8192 | `gfx906/radeonvii` | `llamacpp` | `experimental` | Persistent runtime `registry.harbor.lan/flexinfer/runtime@sha256:f0537a5498ca0ac0afe01a22413e2fa3bc36e0629d9d423960dd0c5572f7cc2b`; standalone profile image `registry.harbor.lan/library/llamacpp:rocm-gfx906-patched-v3` digest TBD | `HF://rippertnt/Qwen3-1.7B-Q4_K_M-GGUF` / `qwen3-1.7b-q4_k_m.gguf` | Cache prefetched and ready on 2026-05-16. MR !394 fixed runtime-manager fallback from stale absolute backend paths to `PATH`; MR !395 added the missing live `runtime:rocm-gfx906` publish job. Master pipeline 9917 published `runtime:rocm-gfx906-a21153d4` at the prior digest and proved command fallback (`/opt/src/.../llama-server` resolved to `/opt/llamacpp/bin/llama-server`). MR !397 separated the flexinfer-runtime API port (`8080`) from the runtime-managed backend port (`8000`); master pipeline 9946 published `runtime:rocm-gfx906-6c92809b` at the previous digest. MR !399 changed the canary to Local cache and staged the GGUF under `/models/flexinfer-system/qwen3-1p7b-tools-radeonvii/qwen3-1.7b-q4_k_m.gguf`. MR !400 hardened llama.cpp launch args, and master pipeline 9980 published `runtime:rocm-gfx906-d3a86071` at this digest. | Port fix rollout passed its contract: live args showed `--port 8000`. MR !399 rollout proved the Local cache and file exist, but runtime launch still received the model directory and then hit the existing `hipMemGetInfo` ROCm abort. MR !400 teaches the llama.cpp backend to append `config.ggufFile`/`modelFile` when handed a cache directory; pending live rollout and one-word smoke. Keep `tool-router` and `qwen3-1.7b` aliases only; do not make this the default chat route unless coherence and latency pass. | After backend GGUF launch patch + runtime digest rollout: `kubectl -n flexinfer-system port-forward svc/flexinfer-proxy 8080:80`; `curl -sS http://127.0.0.1:8080/model/qwen3-1p7b-tools-radeonvii/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"qwen3-1.7b-tools","messages":[{"role":"user","content":"Reply with exactly one word: blue"}],"max_tokens":8,"temperature":0}'` | `registry.harbor.lan/flexinfer/runtime@sha256:b36bb0ab008efa3d8a127cdf7cd9813c8ad88ddf7d62d8736bc6ad0976fe20f0` | RG-4 / `.loom/real-hardware-platform-improvements-plan.md` Slice 4; MR !394; MR !395; MR !397; MR !399; MR !400; pipelines 9917, 9946, 9976, and 9980; 2026-05-16 live failure evidence below | `pending` |
