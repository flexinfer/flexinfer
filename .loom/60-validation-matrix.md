...
| `GPUProfile` vLLM defaults consumer (2026-05-17) | n/a | `gfx1100/7900xtx+5930k`, `gfx906/radeonvii` | `vllm` | `n/a` | n/a | n/a | Controller-managed Deployments and runtime-managed load payloads now apply `GPUProfile.spec.backends.vllm.vllm.defaults` for missing `enforceEager` and `kvCacheDtype` config keys while preserving explicit Model config. `cudagraphMode: NONE` maps to eager mode for older vLLM builds that do not expose a stable mode flag. Focused validation: `go test ./backend ./pkg/runtime ./controllers` and `git diff --check`. | none for config-default consumer; live gfx906 vLLM canary remains a follow-up before any support-level promotion. | `go test ./backend ./pkg/runtime ./controllers` | Revert this MR to return vLLM defaults to schema-only behavior. | `.loom/brainstorm-gpuprofile-vllm-defaults-2026-05-17.md`; `.loom/ralph-gpuprofile-vllm-defaults-2026-05-17.md`; `backend/gpu_compat.go`; `pkg/runtime/payload.go`; `controllers/model_backend.go` | `promote` |
