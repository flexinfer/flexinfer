...
| `qwen3-1p7b-vllm-radeonvii` vLLM canary REVIVED on unified multi-arch image (2026-06-09; first HTTP 200 on this lane) | 2048 | `gfx906/radeonvii` | `vllm` | `experimental` | `registry.harbor.lan/flexinfer/vllm:rocm6.3.4-multiarch-serve@sha256:2f3fe30651ae1cf52b1af12e5db2ad03ed4d36fa1c2b9da64d0e83da3b907920` — vLLM 0.6.3.post1 unified gfx906;gfx1100 serving image, `-serve` derivative (base `@sha256:803399d8…`) baking the request-path guided-decoding short-circuit (outlines → missing pyairports; 2 entry points asserted at build) + the ray `num_gpus` kwarg drop; `build/Dockerfile.vllm-rocm-multiarch-serve` + `build/build-vllm-multiarch-serve.sh` | `HF://facebook/opt-125m`; Local cache (flash-loader) | **Live smoke PASS 2026-06-09 (claude-code).** Full alias-driven serverless path through flexinfer-proxy after re-registering `serviceLabels`/`litellm.aliases` (de-registered in 458f9d84; the transient ~30s soak prober is gone — repo+gitops+cluster swept, no CronJob/blackbox Probe/soak pod claims the aliases): alias `vllm-canary` resolved immediately, cold start ≈2m15s (pod schedule + flash-loader + engine init), Model `phase=Ready`, deterministic completion `"The color of the sky is" → " so beautiful.\nI love it."` (temperature 0, HTTP 200, served via `/v1/completions` body-model alias), then idle scale-back-to-zero at the 10m timeout. Engine evidence: `Overriding num_gpu_blocks=21208 with num_gpu_blocks_override=512` — the profile-derived 21208 blocks ≈ 11.4 GiB single KV `torch.zeros` is exactly the Vega20 (no-VMM) `HIP invalid argument` pathology root-caused same day (TOY-KILLTEST-VERDICT C/D variants; D5 override PASS), sidestepped by `numGpuBlocksOverride: 512` in Model config; `Graph capturing finished in 1 secs` — graph mode live (`enforceEager: false`; eager-only constraint was specific to the retired 0.7.3 diagnostic build); request logged with `GuidedDecodingParams(...)` and served — no outlines/pyairports import crash. | none | in-cluster curl Pod (curlimages/curl): wait for `vllm-canary` in `http://flexinfer-proxy.flexinfer-system.svc.cluster.local/v1/models`, then POST `/v1/completions` `{"model":"vllm-canary","prompt":"The color of the sky is","max_tokens":16,"temperature":0}` (-m 900 covers 15m coldStartTimeout) | previous pin `vllm:rocm-gfx906@sha256:2139c92b…` via git history of `deploy/gpuprofiles/gfx906.yaml`; de-registered alias state = commit 458f9d84 | `.loom/local/validation/f5-3way-2026-06-09/TOY-KILLTEST-VERDICT.md`; `deploy/gpuprofiles/gfx906.yaml`; `deploy/models/qwen3-1p7b-vllm-radeonvii.yaml`; `build/Dockerfile.vllm-rocm-multiarch-serve`; row above (prerequisites + feasibility-only pivot — superseded for the serving path by the community-fork lineage unified image) | `conditional` (import-and-serve proven end-to-end incl. cold-start + idle-down; keep `support: experimental` until a real workload soaks on the lane) |
