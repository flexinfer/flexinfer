...
| `qwen3-1p7b-tools-radeonvii` GGUF tool-router | 8192 | `gfx906/radeonvii` | `llamacpp` | `experimental` | Persistent runtime `registry.harbor.lan/flexinfer/runtime@sha256:a536d25f8b0f154e16c080ce852da6f1430783bedcb53e7098cee3a9baa264c5`; standalone profile image `registry.harbor.lan/library/llamacpp:rocm-gfx906-patched-v3` digest TBD | `HF://rippertnt/Qwen3-1.7B-Q4_K_M-GGUF` / `qwen3-1.7b-q4_k_m.gguf` | Cache prefetched and ready on 2026-05-16. MR !394 fixed runtime-manager fallback from stale absolute backend paths to `PATH`; MR !395 added the missing live `runtime:rocm-gfx906` publish job. Master pipeline 9917 published `runtime:rocm-gfx906-a21153d4` at this digest. The rollout proved command fallback (`/opt/src/.../llama-server` resolved to `/opt/llamacpp/bin/llama-server`) and exposed the next blocker: runtime-managed llama.cpp was still binding `0.0.0.0:8080`, colliding with the flexinfer-runtime API. | Port-collision fix is in progress: runtime-managed backends whose standalone port equals the runtime API port must bind the runtime backend port (`8000`) and report that port through runtime status, controller Endpoints, and proxy direct-load targets. One-word proxy smoke still required before alias promotion. Keep `tool-router` and `qwen3-1.7b` aliases only; do not make this the default chat route unless coherence and latency pass. | After patched runtime rollout: `kubectl -n flexinfer-system port-forward svc/flexinfer-proxy 8080:80`; `curl -sS http://127.0.0.1:8080/model/qwen3-1p7b-tools-radeonvii/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"qwen3-1.7b-tools","messages":[{"role":"user","content":"Reply with exactly one word: blue"}],"max_tokens":8,"temperature":0}'` | `registry.harbor.lan/flexinfer/runtime@sha256:94045d0ca4b12deb3c46bb22070f67bfedad8b719bb992e5d3ce128ad27ad597` | RG-4 / `.loom/real-hardware-platform-improvements-plan.md` Slice 4; MR !394; MR !395; pipeline 9917; 2026-05-16 live failure evidence below | `pending` |
...
- The active fix keeps standalone llama.cpp on port `8080`, but injects and
  routes the runtime-managed backend on port `8000`, matching the runtime
  DaemonSet's `backend` container port.
