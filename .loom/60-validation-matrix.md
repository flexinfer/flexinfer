...
| `qwen3-1p7b-vllm-radeonvii` vLLM canary prerequisites (2026-05-17) | 4096 | `gfx906/radeonvii` | `vllm` | `experimental` | Persistent runtime `registry.harbor.lan/flexinfer/runtime@sha256:cbe1157c2fb6a24fc67e901bec92a72bbf16498a86ad1a064ce9bf4db1f2ddf4`; standalone vLLM image `registry.harbor.lan/flexinfer/vllm:rocm-gfx906@sha256:beadf394fc81c031799799f5d965664e419e5f3ffb4c5873a9d7677f0e1e06b8` | `HF://Qwen/Qwen3-1.7B`; Local cache at `/models/flexinfer-system/qwen3-1p7b-vllm-radeonvii` | RALPH fixed the canary prerequisites: DNS-label-safe resource name, `Local` cache, canary-scoped aliases, standalone image path for gfx906 vLLM, and image-pull-secret propagation for controller-created model Deployments. On 2026-05-18, the Radeon VII k3s containerd image store was moved from root LVM to the NVMe-backed `/mnt/nvme/longhorn/k3s-containerd/containerd` bind mount. A follow-up proxy demand reached the standalone vLLM image pull path without `DiskPressure=True`; the cache stage completed and the Model returned to `Idle` after cleanup. | No HTTP 200 vLLM smoke yet. The image-store capacity blocker is cleared, but the current blocker is Harbor auth for `registry.harbor.lan/flexinfer/vllm`: `harbor-creds` and a temporary dockerconfig built from `harbor-oci-creds` both returned `401 Unauthorized` for the pinned digest. Next slice must grant the cluster pull robot access to `flexinfer/vllm` or republish the canary image into a project readable by the existing pull secret, then rerun the canary. | After fixing Harbor access, run `kubectl -n flexinfer-system run smoke-gfx906-vllm --rm -i --restart=Never --image=curlimages/curl:8.11.1 -- curl -sS -m 900 -X POST http://flexinfer-proxy.flexinfer-system.svc.cluster.local/model/qwen3-1p7b-vllm-radeonvii/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"qwen3-1.7b-vllm","messages":[{"role":"user","content":"The color of the sky is"}],"max_tokens":16,"temperature":0}'` | Delete `Model/qwen3-1p7b-vllm-radeonvii`, restore `qwen3-1p7b-tools-radeonvii` `minReplicas: 1`, and recycle `flexinfer-runtime-gfx906` if `/readyz` hangs. | `.loom/ralph-gfx906-vllm-smoke-2026-05-17.md`; `deploy/models/qwen3-1p7b-vllm-radeonvii.yaml`; `deploy/gpuprofiles/gfx906.yaml`; `deploy/system/values-k3s.yaml`; evidence dir `/tmp/flexinfer-gfx906-vllm-fixed-smoke-20260517T193341Z`; live imagefs/auth follow-up 2026-05-18 | `block` |
