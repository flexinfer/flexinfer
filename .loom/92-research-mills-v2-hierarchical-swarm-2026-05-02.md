# Research: Mills v2 — Hierarchical Agent Swarm Coordination

> Date: 2026-05-02
> Status: Draft v1 — research baseline for the next layer of Loom Mills
> Scope: Promote Mills from a flat two-tier (Council → Pipeline) to a true **three-tier hierarchical swarm** (Council → Squads → Pipeline) with adversarial audit, cross-repo federation, and adaptive routing. Carries forward the v1.1 deferrals from `.loom/91-implementation-plan-agent-swarm-council-pipeline-2026-04-25.md` §"Open items".
...
| 3 | 3.8 `loom mills council/backlog/eval` CLI | shipped | `c04d0e15` |
...
| 5 | 5.2 HUD `Mills` view (4 panels) | shipped | `4b2f3418` |
| 5 | 5.3 Overview KPI cards | shipped | `54bc0787` |
| 5 | 5.4 Grafana dashboard | (verify in `platform/gitops/monitoring/dashboards/mills.json`) |
...
| 6 | 6.5 Docs + runbook (`docs/MILLS.md`, `docs/MILLS_RUNBOOK.md`, `mcp/skills/mills-ops/`) | **not shipped** |
| 6 | 6.6 Default-on flip + production rollout playbook | **not shipped** |

Code surface today (verified via `wc -l pkg/mills/...`): ~18.2k lines under `pkg/mills`, 14 handler files under `cmd/loom-mills-operator/`, 4 panels under `internal/hud/frontend/src/lib/components/Mills/`, 1 MentatLab template, 1 Grafana dashboard slot.
...
The aggregate effect is to turn the mills from "five agents in a daily meeting" into "three teams plus an external auditor with continuous standups," which is the smallest org structure that is empirically known to ship complex software at scale.
...
| **Recursive decomposition** | Single fan-out level (`pkg/mills/pipeline/integrator.go`). Sub-slices must fit one worktree each. | Bounded recursion (depth ≤ 2): a fan-out worker can itself fan out, with budget attributed up the tree and cycle detection. |
...
## 6. The shape of Mills v2

```
                ┌─────────────────────────────────────────────────────────────┐
                │                       LOOM MILLS v2                          │
                │                  (loom-mills-operator @ k3s)                 │
                │                                                             │
   ROADMAP +    │   ┌────────────────┐                                        │
...
1. **Own a domain manifest.** Each squad has a YAML (`platform/gitops/k3s/mills/squads/<name>.yaml`) declaring:
...
3. **Use a different rubric.** The audit rubric (in `pkg/mills/audit/rubric/audit_v1.md`) emphasizes failure modes the editor judge does not score: hidden assumptions, deletion-by-omission, missing tests for edge cases the spec mentions, accidental coupling between slices, plan-vs-actual drift on merged work.
...
3. **Repo registry.** `platform/gitops/k3s/mills/repos.yaml` lists known repos with their GitLab project IDs, default branch, and an `auto_merge` flag. Gated behind `policy.cross_repo.enabled: false` until v2.1.
...
1. **Pipeline workers can call back into the operator.** A `tools/spawn-driver` worker has access to `mcp-mills::pipeline_subrun_create(parent_run_id, slice_spec, depth)` (new MCP tool).
...
2. **Output: policy diffs.** A scheduled job (Sunday 0500) produces a markdown diff suggesting policy changes (e.g., "relax `coverage` gate from required to advisory for `internal/hud/frontend/` based on 100/100 pass rate"). Diff is committed as `.loom/mills/policy_proposals/<date>.md`.
...
```
loom_mills_squad_runs_total{squad, outcome="planned|delegated|self_vetoed|merged|failed"}
loom_mills_squad_success_rate{squad, path_class}
loom_mills_squad_budget_usd_total{squad}
loom_mills_audit_findings_total{severity="info|warn|critical", subject_kind="council|pipeline"}
loom_mills_audit_survival_rate{subject_kind, window="day|week"}
loom_mills_cross_repo_runs_total{outcome="success|partial|reverted"}
loom_mills_cross_repo_atomicity_violations_total
loom_mills_council_debate_rounds_total{round_index}
loom_mills_council_debate_cost_usd_total{round_index}
loom_mills_pipeline_recursion_depth_histogram
loom_mills_policy_proposals_total{kind="relax|tighten", applied="auto|human|pending"}
```
...
| V2-D2 | Squad memory storage | (a) New tables in canonical store (b) Reuse `agent-context` memory APIs | (a) — keep mills self-contained; agent-context is for agent sessions, mills squads are operator-owned |
...
| V2-D8 | New tables location | new file `pkg/mills/store/migrations/002_v2.sql` / extend 001 | new file to keep migrations append-only and reviewable |
...
| V2-D10 | Mobile parity (v1.1 deferral #1) | in v2 / defer | in v2 — Mills HUD view exists; mobile parity is one screen, ships in slice 5.x |
...
- Command: `git log --grep='mills\|Mills' --since='2026-04-25' --oneline`
- `pkg/mills/` (18.2k Go LOC; verified via `wc -l pkg/mills/**/*.go`)
- `cmd/loom-mills-operator/` (14 handler files)
- `cmd/mcp-mentatlab/templates/mills-default-pipeline.yaml`
- `internal/hud/frontend/src/lib/components/Mills/` (4 panels)
