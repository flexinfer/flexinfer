...
func TestHandleRegressionAlert_RequiresAdmin(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/mills/alerts/regression", bytes.NewBufferString("{}"))
...
func TestHandleRegressionAlert_DecodesPayloadAndCorrelates(t *testing.T) {
	...
	if err := op.store.Backlog.Put(context.Background(), &store.BacklogItem{
		ID: "MILLS-A", Title: "test A", State: store.BacklogMerged,
	...
	if err := op.store.Pipeline.PutRun(context.Background(), &store.PipelineRun{
		ID: "PIPE-A", BacklogID: "MILLS-A", Template: "mills-default-pipeline",
	...
	req := httptest.NewRequest(http.MethodPost, "/api/mills/alerts/regression", bytes.NewReader(body))
...
func TestHandleRegressionAlert_BadPayload(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/mills/alerts/regression", bytes.NewBufferString("{invalid"))
	req.Header.Set("Authorization", "Bearer secret")
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, req)
