...
func (o *operator) httpMux() *http.ServeMux {
	...
	mux.HandleFunc("POST /api/mills/council/run", requireAdmin(o.handleCouncilRun))
	mux.HandleFunc("POST /api/mills/council/dryrun", requireAdmin(o.handleCouncilDryrun))
	...
	mux.HandleFunc("POST /api/mills/pipeline/runs/{backlog_id}/start", requireAdmin(o.handlePipelineStart))
	mux.HandleFunc("POST /api/mills/pipeline/runs/{id}/pause", requireAdmin(o.handlePipelinePause))
	mux.HandleFunc("POST /api/mills/pipeline/runs/{id}/resume", requireAdmin(o.handlePipelineResume))
	...
	mux.HandleFunc("POST /api/mills/backlog", requireAdmin(o.handleBacklogCreate))
	mux.HandleFunc("POST /api/mills/backlog/sync", requireAdmin(o.handleBacklogSync))
	...
	mux.HandleFunc("POST /api/mills/eval/run-cross", requireAdmin(o.handleEvalRunCross))
	...
	mux.HandleFunc("POST /api/mills/audit/run", requireAdmin(o.handleAuditRun))
	...
	mux.HandleFunc("POST /api/mills/cross-repo/runs/{id}/abort", requireAdmin(o.handleCrossRepoAbort))
	...
	mux.HandleFunc("POST /api/mills/alerts/regression", requireAdmin(o.handleRegressionAlert))

	// Adaptive policy proposals (Phase 7 slice 7.2).
	mux.HandleFunc("GET /api/mills/policy/proposals", o.handlePolicyProposalsList)
	mux.HandleFunc("POST /api/mills/policy/proposals/{id}/apply", requireAdmin(o.handlePolicyProposalApply))
	mux.HandleFunc("POST /api/mills/policy/proposals/{id}/reject", requireAdmin(o.handlePolicyProposalReject))
