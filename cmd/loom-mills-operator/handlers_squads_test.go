...
const fixtureSquadHUD = `apiVersion: mills.loom.dev/v1
kind: Squad
metadata:
  name: hud-frontend
spec:
...
const fixtureSquadGitops = `apiVersion: mills.loom.dev/v1
kind: Squad
metadata:
  name: gitops
spec:
...
func TestHandleSquadsList_EmptyReturns200WithEmptyArray(t *testing.T) {
	...
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/mills/squads", nil))
...
func TestHandleSquadGet_Returns404ForUnknown(t *testing.T) {
	...
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/mills/squads/does-not-exist", nil))
...
func TestHandleSquadGet_ReturnsDetail(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/mills/squads/hud-frontend", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
	body := rec.Body.String()
...
func TestHandleSquadMemory_HonorsLimitCap(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/mills/squads/hud-frontend/memory?limit=3", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
...
func TestHandleSquadMemory_KindFilter(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/mills/squads/hud-frontend/memory?kind=tech_debt", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
	body := rec.Body.String()
...
func TestHandleSquadRouteTest_RequiresAdminToken(t *testing.T) {
	...
	body := bytes.NewBufferString(`{"backlog_id":"MILLS-X-1"}`)
	req := httptest.NewRequest(http.MethodPost,
		"/api/mills/squads/hud-frontend/route-test", body)
...
func TestHandleSquadRouteTest_HappyPath(t *testing.T) {
	...
	if err := op.store.Backlog.Put(ctx, &store.BacklogItem{
		ID: "MILLS-RT-001", Title: "route-test", State: store.BacklogQueued,
	...
	body := bytes.NewBufferString(`{"backlog_id":"MILLS-RT-001"}`)
	req := httptest.NewRequest(http.MethodPost,
		"/api/mills/squads/hud-frontend/route-test", body)
