// cmd_hive_crossrepo.go implements `loom hive cross-repo` subcommands.
// Cross-repo runs coordinate atomic merges across multiple GitLab projects
// for a single backlog item (Hive v2 spec §"Cross-repo coordination flow").
...
// newHiveCrossRepoCmd is the parent for cross-repo run operations:
//   - list  (read)
//   - show  (read)
//   - abort (admin: requires a token)
func newHiveCrossRepoCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "cross-repo",
...
Abort requires LOOM_ADMIN_TOKEN or --admin-token (admin-gated, marks run failed).`,
	...
	cmd.AddCommand(
		newHiveCrossRepoListCmd(),
		newHiveCrossRepoShowCmd(),
		newHiveCrossRepoAbortCmd(),
	)
	return cmd
}

// newHiveCrossRepoListCmd renders the run inventory table. The operator
// supports filtering by state and backlog id; --limit caps response size.
func newHiveCrossRepoListCmd() *cobra.Command {
...
// newHiveCrossRepoShowCmd renders a single run with its per-repo detail
// table. Backed by GET /api/hive/cross-repo/runs/{id}.
func newHiveCrossRepoShowCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/api/hive/cross-repo/runs/" + url.PathEscape(id)
			emitJSON, _ := cmd.Flags().GetBool("json")
			if emitJSON {
				var raw json.RawMessage
				if err := client.get(ctx, path, &raw); err != nil {
...
// newHiveCrossRepoAbortCmd hits POST /api/hive/cross-repo/runs/{id}/abort.
...
func newHiveCrossRepoAbortCmd() *cobra.Command {
	var assumeYes bool
	cmd := &cobra.Command{
		Use:   "abort <id>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/api/hive/cross-repo/runs/" + url.PathEscape(id) + "/abort"
			emitJSON, _ := cmd.Flags().GetBool("json")
			if emitJSON {
				var raw json.RawMessage
...
func wrapCrossRepoHTTPErr(c *hiveClient, subject string, err error) error {
	if err == nil {
		return nil
	}
	msg := err.Error()
	...
	case strings.Contains(msg, "operator returned 404"):
		return fmt.Errorf("%w\nhint: %q not found — try `loom hive cross-repo list` to see active runs",
			err, subject)
