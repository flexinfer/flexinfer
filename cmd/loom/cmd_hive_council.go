// cmd_hive_council.go implements the `loom hive {council,backlog,eval}`
...
func renderCouncilRun(w interface{ Write([]byte) (int, error) }, base string, dryrun bool, r councilRunResponse) {
	...
	fmt.Fprintf(w, "Loom Hive council %s @ %s\n", mode, base)
...
// newHiveCouncilRunsCmd lists past council runs (read-only).
func newHiveCouncilRunsCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if emitJSON {
				var raw json.RawMessage
				if err := client.get(ctx, "/api/hive/council/runs", &raw); err != nil {
					return wrapHiveErr(client, err)
				}
				_, err := fmt.Fprintln(cmd.OutOrStdout(), string(raw))
				return err
			...
			if err := client.get(ctx, "/api/hive/council/runs", &runs); err != nil {
				return wrapHiveErr(client, err)
			}
			if len(runs) == 0 {
...
func newHiveBacklogCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "backlog",
		Short: "Backlog snapshot (read-only list + per-item detail)",
	}
	cmd.AddCommand(
		newHiveBacklogListCmd(),
		newHiveBacklogGetCmd(),
	)
	return cmd
}

func newHiveBacklogListCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if emitJSON {
				var raw json.RawMessage
				if err := client.get(ctx, "/api/hive/backlog", &raw); err != nil {
					return wrapHiveErr(client, err)
				}
				_, err := fmt.Fprintln(cmd.OutOrStdout(), string(raw))
				return err
			...
			if err := client.get(ctx, "/api/hive/backlog", &items); err != nil {
				return wrapHiveErr(client, err)
			}
			if len(items) == 0 {
...
func newHiveBacklogGetCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "get <id>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var raw json.RawMessage
			if err := client.get(ctx, "/api/hive/backlog/"+args[0], &raw); err != nil {
				return wrapHiveErr(client, err)
			}
			_, err = fmt.Fprintln(cmd.OutOrStdout(), string(raw))
			return err
...
func newHiveEvalCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "eval",
		Short: "Eval scores (Loop A artifact judge, Loop B per-merge attribution)",
	}
	cmd.AddCommand(newHiveEvalListCmd())
	return cmd
}

func newHiveEvalListCmd() *cobra.Command {
	var since string
	cmd := &cobra.Command{
		Use:   "list",
