// cmd_mills_squads.go implements the `loom mills squads` subcommands. Squads
// are the per-area execution units (hud-frontend, gitops, ...) introduced in
// Mills v2 (see .loom/93-product-spec-mills-v2-hierarchical-swarm-2026-05-02.md
...
// admin operations (route-test). We also fall back to LOOM_MILLS_TOKEN for
// backward compatibility with the rest of `loom mills`.
const (
	squadsAdminTokenEnv  = "LOOM_ADMIN_TOKEN"
	squadsLegacyTokenEnv = "LOOM_MILLS_TOKEN"
)

// newMillsSquadsCmd is the parent for the four squads operations:
//   - list       (read)
//   - show       (read)
//   - memory     (read, paginated)
//   - route-test (admin: requires a token)
func newMillsSquadsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "squads",
...
Mutating commands (route-test) require LOOM_ADMIN_TOKEN or --admin-token.`,
	...
	cmd.AddCommand(
		newMillsSquadsListCmd(),
		newMillsSquadsShowCmd(),
		newMillsSquadsMemoryCmd(),
		newMillsSquadsRouteTestCmd(),
	)
	return cmd
}

// resolveSquadsClient is a thin wrapper around resolveMillsClient that lets
// `--admin-token` and $LOOM_ADMIN_TOKEN take precedence over the legacy
// $LOOM_MILLS_TOKEN. Read commands also accept the legacy token (handy for
// users who already export it for council work).
func resolveSquadsClient(cmd *cobra.Command) (*millsClient, error) {
	...
	if tok == "" {
		// Fall back to the same env the rest of `loom mills` uses; the
		// millsClient already has it loaded, so respect that.
		tok = client.token
	}
	client.token = tok
	return client, nil
...
// squadSummary is the row shape `loom mills squads list` renders. The spec
...
// newMillsSquadsListCmd renders the inventory table. Empty list prints
// "(no squads)" so we never panic on an empty deployment.
func newMillsSquadsListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if emitJSON {
				...
				if err := client.get(ctx, "/api/mills/squads", &raw); err != nil {
			...
			if err := client.get(ctx, "/api/mills/squads", &squads); err != nil {
				return wrapMillsErr(client, err)
			}
			if len(squads) == 0 {
...
// squadDetail is the full per-squad payload `loom mills squads show <name>`
...
// newMillsSquadsShowCmd renders one squad's detail block. 404 surfaces as a
// user-readable message that names the squad — much friendlier than the
// raw "operator returned 404" mills client default.
func newMillsSquadsShowCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "show <name>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/api/mills/squads/" + url.PathEscape(name)
			emitJSON, _ := cmd.Flags().GetBool("json")
			if emitJSON {
				var raw json.RawMessage
...
// newMillsSquadsMemoryCmd lists working-memory entries for a squad. Sorted
...
func newMillsSquadsMemoryCmd() *cobra.Command {
	var kind string
	var limit int
	cmd := &cobra.Command{
		Use:   "memory <name>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/api/mills/squads/" + url.PathEscape(name) + "/memory"
...
// newMillsSquadsRouteTestCmd simulates the routing decision for one
...
// prompt suggests (`/api/mills/squads/route-test`) and decode best-effort.
func newMillsSquadsRouteTestCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/api/mills/squads/route-test"
...
func wrapSquadHTTPErr(c *millsClient, subject string, err error) error {
	if err == nil {
		return nil
	}
	msg := err.Error()
	...
	case strings.Contains(msg, "operator returned 404"):
		return fmt.Errorf("%w\nhint: %q not found — try `loom mills squads list` to see available squads",
			err, subject)
	}
