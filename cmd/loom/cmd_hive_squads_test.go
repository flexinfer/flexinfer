...
// subset of the loom-hive-operator REST surface. Tests can switch
...
func fakeSquadsOperator(t *testing.T, opts fakeSquadsOpts) (*httptest.Server, *fakeSquadsState) {
	...
	mux.HandleFunc("GET /api/hive/squads", func(w http.ResponseWriter, r *http.Request) {
		state.listCalls++
		state.lastAuthHeader = r.Header.Get("Authorization")
		writeJSON(w, opts.listStatus, opts.listResponse)
	})

	mux.HandleFunc("GET /api/hive/squads/{name}", func(w http.ResponseWriter, r *http.Request) {
		state.showCalls++
		state.lastAuthHeader = r.Header.Get("Authorization")
		writeJSON(w, opts.showStatus, opts.showResponse)
	})

	mux.HandleFunc("GET /api/hive/squads/{name}/memory", func(w http.ResponseWriter, r *http.Request) {
		state.memoryCalls++
		state.lastAuthHeader = r.Header.Get("Authorization")
	...
	mux.HandleFunc("POST /api/hive/squads/route-test", func(w http.ResponseWriter, r *http.Request) {
...
func TestHiveSquadsList_Table(t *testing.T) {
	...
	out, err := runHive(t, srv, "squads", "list")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveSquadsList_Empty(t *testing.T) {
...
func TestHiveSquadsList_NullJSONResponse(t *testing.T) {
...
func TestHiveSquadsList_JSONPassthrough(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{
		listResponse: []map[string]any{
			{"name": "hud-frontend", "paths": []string{"web/**"}, "success_rate": 1.0, "in_flight": 0},
		},
	})
	out, err := runHive(t, srv, "squads", "list", "--json")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestHiveSquadsShow_Detail(t *testing.T) {
	srv, state := fakeSquadsOperator(t, fakeSquadsOpts{
	...
	out, err := runHive(t, srv, "squads", "show", "hud-frontend")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveSquadsShow_404Friendly(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{
		showStatus:   http.StatusNotFound,
		showResponse: map[string]string{"error": "not found"},
	})
	_, err := runHive(t, srv, "squads", "show", "ghosts")
	if err == nil {
		t.Fatal("expected error on 404")
	}
	msg := err.Error()
	for _, want := range []string{`"ghosts"`, "loom hive squads list"} {
...
func TestHiveSquadsMemory_Table(t *testing.T) {
	srv, state := fakeSquadsOperator(t, fakeSquadsOpts{
	...
	out, err := runHive(t, srv, "squads", "memory", "hud-frontend",
...
func TestHiveSquadsMemory_Empty(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{
		memoryResponse: []map[string]any{},
	})
	out, err := runHive(t, srv, "squads", "memory", "hud-frontend")
...
func TestHiveSquadsRouteTest_RequiresAdminToken(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{})
	// No token set anywhere — should fail before the request hits the wire.
	t.Setenv("LOOM_HIVE_TOKEN", "")
	t.Setenv("LOOM_ADMIN_TOKEN", "")

	_, err := runHive(t, srv, "squads", "route-test", "HIVE-2026-04-26-001")
	if err == nil {
...
func TestHiveSquadsRouteTest_HappyPath_FlagToken(t *testing.T) {
	srv, state := fakeSquadsOperator(t, fakeSquadsOpts{
		requireAuth:   true,
		expectedToken: "tok-flag",
		routeResponse: map[string]any{
			"backlog_id": "HIVE-2026-04-26-001",
			"squad":      "hud-frontend",
			"confidence": 0.83,
	...
	out, err := runHive(t, srv, "squads", "--admin-token", "tok-flag",
		"route-test", "HIVE-2026-04-26-001")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
	...
	if got := state.lastRouteBody["backlog_id"]; got != "HIVE-2026-04-26-001" {
		t.Errorf("backlog_id round-trip: got %v", got)
	}
	for _, want := range []string{
		"Route test for HIVE-2026-04-26-001",
		"squad:      hud-frontend",
		"confidence: 0.83",
		"matched 3/4 paths",
		"candidates:",
...
func TestHiveSquadsRouteTest_AdminTokenEnvWins(t *testing.T) {
	// LOOM_ADMIN_TOKEN should beat LOOM_HIVE_TOKEN so users following the
	...
	t.Setenv("LOOM_HIVE_TOKEN", "legacy")
	t.Setenv("LOOM_ADMIN_TOKEN", "primary")

	if _, err := runHive(t, srv, "squads", "route-test", "HIVE-X"); err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestHiveSquadsRouteTest_LegacyTokenFallback(t *testing.T) {
	srv, state := fakeSquadsOperator(t, fakeSquadsOpts{
		requireAuth:   true,
...
func TestHiveSquadsRouteTest_401Friendly(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{
		requireAuth:   true,
	...
	_, err := runHive(t, srv, "squads", "route-test", "HIVE-X")
	if err == nil {
...
func TestHiveSquadsRouteTest_JSONPassthrough(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{
		requireAuth:   true,
		expectedToken: "tok",
	...
	out, err := runHive(t, srv, "squads", "route-test", "HIVE-Y", "--json")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestHiveSquads_HelpListsSubcommands(t *testing.T) {
	srv, _ := fakeSquadsOperator(t, fakeSquadsOpts{})
	// `--help` should not require any HTTP call; we still pass --operator-url
	// because runHive sets it unconditionally.
	out, err := runHive(t, srv, "squads", "--help")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
