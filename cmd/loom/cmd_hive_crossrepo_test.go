...
func fakeCrossRepoOperator(t *testing.T, opts fakeCrossRepoOpts) (*httptest.Server, *fakeCrossRepoState) {
	...
	mux.HandleFunc("GET /api/hive/cross-repo/runs", func(w http.ResponseWriter, r *http.Request) {
		state.listCalls++
	...
	mux.HandleFunc("GET /api/hive/cross-repo/runs/{id}", func(w http.ResponseWriter, r *http.Request) {
		state.showCalls++
		state.lastAuthHeader = r.Header.Get("Authorization")
	...
	mux.HandleFunc("POST /api/hive/cross-repo/runs/{id}/abort", func(w http.ResponseWriter, r *http.Request) {
		state.abortCalls++
		state.lastAuthHeader = r.Header.Get("Authorization")
...
// runHiveWithStdin is a stdin-aware variant of runHive (defined in
// cmd_hive_council_test.go) so the abort confirmation prompt can be driven
// from the test.
func runHiveWithStdin(t *testing.T, srv *httptest.Server, stdin string, args ...string) (string, error) {
	t.Helper()
	cmd := newHiveCmd()
...
func TestHiveCrossRepoList_Table(t *testing.T) {
	...
	out, err := runHive(t, srv, "cross-repo", "list")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveCrossRepoList_StateFilter(t *testing.T) {
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		listResponse: map[string]any{"runs": []any{}, "total": 0, "limit": 10},
	})
	if _, err := runHive(t, srv, "cross-repo", "list", "--state", "merged,reverted", "--limit", "10"); err != nil {
...
func TestHiveCrossRepoList_BacklogFilter(t *testing.T) {
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		listResponse: map[string]any{"runs": []any{}, "total": 0, "limit": 50},
	})
	if _, err := runHive(t, srv, "cross-repo", "list", "--backlog-id", "HIVE-XYZ"); err != nil {
		t.Fatalf("execute: %v", err)
	}
	if !strings.Contains(state.lastListQuery, "backlog_id=HIVE-XYZ") {
		t.Errorf("backlog filter not forwarded; query=%q", state.lastListQuery)
	}
}

func TestHiveCrossRepoList_InvalidStateRejected(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		listResponse: map[string]any{"runs": []any{}, "total": 0, "limit": 50},
	})
	_, err := runHive(t, srv, "cross-repo", "list", "--state", "marshmallow")
	if err == nil {
...
func TestHiveCrossRepoList_Empty(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		listResponse: map[string]any{"runs": []any{}, "total": 0, "limit": 50},
	})
	out, err := runHive(t, srv, "cross-repo", "list")
...
func TestHiveCrossRepoList_JSONPassthrough(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		listResponse: map[string]any{
			"runs": []map[string]any{
				{"id": "XR-1", "backlog_item_id": "HIVE-1", "state": "open"},
			},
			"total": 1,
		},
	})
	out, err := runHive(t, srv, "cross-repo", "list", "--json")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestHiveCrossRepoShow_Detail(t *testing.T) {
	mr := int64(42)
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
	...
	out, err := runHive(t, srv, "cross-repo", "show", "XR-2026-04-26-001")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveCrossRepoShow_404Friendly(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		showStatus:   http.StatusNotFound,
		showResponse: map[string]string{"error": "not found"},
	})
	_, err := runHive(t, srv, "cross-repo", "show", "ghosts")
	if err == nil {
		t.Fatal("expected error on 404")
	}
	for _, want := range []string{`"ghosts"`, "loom hive cross-repo list"} {
...
func TestHiveCrossRepoAbort_RequiresAdminToken(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{})
	t.Setenv("LOOM_HIVE_TOKEN", "")
	t.Setenv("LOOM_ADMIN_TOKEN", "")

	_, err := runHive(t, srv, "cross-repo", "abort", "XR-1", "--yes")
	if err == nil {
...
func TestHiveCrossRepoAbort_HappyPath_FlagToken(t *testing.T) {
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
	...
	out, err := runHive(t, srv, "cross-repo", "--admin-token", "tok-flag",
		"abort", "XR-1", "--yes")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveCrossRepoAbort_ConfirmPromptYes(t *testing.T) {
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
		expectedToken: "tok",
		abortResponse: map[string]any{
	...
	out, err := runHiveWithStdin(t, srv, "y\n", "cross-repo", "abort", "XR-2")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
	if state.abortCalls != 1 {
...
func TestHiveCrossRepoAbort_ConfirmPromptDecline(t *testing.T) {
	srv, state := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
		expectedToken: "tok",
		abortResponse: map[string]any{
	...
	out, err := runHiveWithStdin(t, srv, "n\n", "cross-repo", "abort", "XR-3")
	if err != nil {
		t.Fatalf("execute: %v\noutput=%q", err, out)
	}
...
func TestHiveCrossRepoAbort_404Friendly(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
		expectedToken: "tok",
	...
	_, err := runHive(t, srv, "cross-repo", "abort", "missing", "--yes")
	if err == nil {
		t.Fatal("expected error on 404")
	}
	for _, want := range []string{`"missing"`, "loom hive cross-repo list"} {
		if !strings.Contains(err.Error(), want) {
...
func TestHiveCrossRepoAbort_409Friendly(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
		expectedToken: "tok",
	...
	_, err := runHive(t, srv, "cross-repo", "abort", "XR-done", "--yes")
	if err == nil {
...
func TestHiveCrossRepoAbort_JSONPassthrough(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{
		requireAuth:   true,
		expectedToken: "tok",
	...
	out, err := runHive(t, srv, "cross-repo", "abort", "XR-J", "--yes", "--json")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
...
func TestHiveCrossRepo_HelpListsSubcommands(t *testing.T) {
	srv, _ := fakeCrossRepoOperator(t, fakeCrossRepoOpts{})
	out, err := runHive(t, srv, "cross-repo", "--help")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
