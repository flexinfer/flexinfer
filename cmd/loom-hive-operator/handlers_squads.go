...
// inline by GET /api/hive/squads and GET /api/hive/squads/{name}.
...
// squadsListEntry is the per-row shape returned by GET /api/hive/squads.
...
// squadDetail is the response shape for GET /api/hive/squads/{name}.
...
func (o *operator) handleSquadRouteTest(w http.ResponseWriter, r *http.Request) {
	if o.squadsLoader == nil {
		http.Error(w, "squads loader not configured (missing LOOM_HIVE_SQUADS_PATH or empty manifest dir)",
