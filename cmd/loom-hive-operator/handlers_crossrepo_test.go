...
func TestHandleCrossRepoList_NoFilterReturnsAll(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-1", "HIVE-A", store.CrossRepoOpen)
	seedCrossRepoRun(t, op, "XR-2", "HIVE-B", store.CrossRepoMerged)
...
func TestHandleCrossRepoList_StateFilter(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs?state=open", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
	var resp crossRepoListResponse
...
func TestHandleCrossRepoList_MultiStateFilter(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-1", "HIVE-A", store.CrossRepoOpen)
	seedCrossRepoRun(t, op, "XR-2", "HIVE-B", store.CrossRepoGatesGreen)
	seedCrossRepoRun(t, op, "XR-3", "HIVE-C", store.CrossRepoMerged)

	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs?state=open,gates_green", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
	var resp crossRepoListResponse
...
func TestHandleCrossRepoList_BadStateReturns400(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs?state=invalid_state", nil))
...
func TestHandleCrossRepoList_BacklogFilter(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-1", "HIVE-A", store.CrossRepoOpen)
	seedCrossRepoRun(t, op, "XR-2", "HIVE-A", store.CrossRepoMerged)
	seedCrossRepoRun(t, op, "XR-3", "HIVE-B", store.CrossRepoOpen)

	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs?backlog_id=HIVE-A", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
	var resp crossRepoListResponse
	...
	if resp.Total != 2 {
		t.Errorf("Total = %d, want 2 (both HIVE-A rows)", resp.Total)
	}
	for _, run := range resp.Runs {
		if run.BacklogItemID != "HIVE-A" {
			t.Errorf("got run for backlog %q, want HIVE-A only", run.BacklogItemID)
		}
	}
}
...
func TestHandleCrossRepoGet_Hit(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-7", "HIVE-Z", store.CrossRepoOpen)

	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs/XR-7", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status: %d body=%s", rec.Code, rec.Body.String())
	}
...
func TestHandleCrossRepoGet_Miss(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet,
		"/api/hive/cross-repo/runs/does-not-exist", nil))
...
func TestHandleCrossRepoAbort_HappyPath(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-A", "HIVE-Q", store.CrossRepoOpen)

	req := httptest.NewRequest(http.MethodPost,
		"/api/hive/cross-repo/runs/XR-A/abort", nil)
...
func TestHandleCrossRepoAbort_TerminalRejected(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-DONE", "HIVE-Q", store.CrossRepoMerged)

	req := httptest.NewRequest(http.MethodPost,
		"/api/hive/cross-repo/runs/XR-DONE/abort", nil)
	req.Header.Set("Authorization", "Bearer secret-abc")
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, req)
...
func TestHandleCrossRepoAbort_PlanningRejected(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-PLAN", "HIVE-Q", store.CrossRepoPlanning)

	req := httptest.NewRequest(http.MethodPost,
		"/api/hive/cross-repo/runs/XR-PLAN/abort", nil)
	req.Header.Set("Authorization", "Bearer secret-abc")
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, req)
	if rec.Code != http.StatusConflict {
...
func TestHandleCrossRepoAbort_UnknownReturns404(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost,
		"/api/hive/cross-repo/runs/does-not-exist/abort", nil)
	req.Header.Set("Authorization", "Bearer secret-abc")
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, req)
	if rec.Code != http.StatusNotFound {
...
func TestHandleCrossRepoAbort_RequiresAdminToken(t *testing.T) {
	...
	seedCrossRepoRun(t, op, "XR-X", "HIVE-Q", store.CrossRepoOpen)

	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodPost,
		"/api/hive/cross-repo/runs/XR-X/abort", nil))
