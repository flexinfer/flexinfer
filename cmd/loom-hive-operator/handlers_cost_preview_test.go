...
func TestCostPreview_ReturnsEstimate_NoAdminToken(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/hive/cost-preview?backlog_id=BACK-PREVIEW", nil)
...
func TestCostPreview_400_MissingBacklogID(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/hive/cost-preview", nil)
	op.httpMux().ServeHTTP(rec, req)
...
func TestCostPreview_404_UnknownBacklog(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/hive/cost-preview?backlog_id=BACK-NOPE", nil)
	op.httpMux().ServeHTTP(rec, req)
