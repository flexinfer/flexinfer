...
func newTestOperator(t *testing.T) (*operator, func()) {
	...
	pm, err := hive.NewPolicyManager(context.Background(), policyPath,
		hive.PolicyManagerOptions{SkipWatch: true})
	if err != nil {
		_ = st.Close()
		t.Fatalf("policy manager: %v", err)
	}
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	op := newOperator(st, pm, hive.NewBudget(pm, hive.NewStoreBudgetReader(st)), logger)
...
func TestStatus_FullResponds(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	op.httpMux().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/hive/status", nil))
	if rec.Code != http.StatusOK {
...
func TestConfig_Validate(t *testing.T) {
	cases := []struct {
		...
		{"db-path missing dir", func(c *Config) { c.DBPath = "hive.db" }, "must include a directory"},
	}
...
func TestConfig_ApplyEnv(t *testing.T) {
	t.Setenv("LOOM_HIVE_DB_PATH", "/tmp/x.db")
	t.Setenv("LOOM_HIVE_POLICY_PATH", "/tmp/policy.yaml")
	t.Setenv("LOOM_HIVE_HTTP_ADDR", ":1234")
	t.Setenv("LOOM_HIVE_METRICS_ADDR", ":5678")
	t.Setenv("LOOM_HIVE_DEBUG", "true")
	t.Setenv("LOOM_HIVE_ENABLED", "false")
