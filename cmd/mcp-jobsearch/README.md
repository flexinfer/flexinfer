...
- `JOBSEARCH_API_TOKEN` (optional, fallback: `JOBSEARCH_BEARER_TOKEN`)
...
`Authorization: Bearer ...` is only injected when a JobSearch token is configured.

## Tooling Model

- Explicit Workflow + CRM tools for common operations.
- Explicit Resume Management tools for:
  - resume composition + variant lifecycle
  - DB-backed resume/version workflows
  - resume chat session workflows
