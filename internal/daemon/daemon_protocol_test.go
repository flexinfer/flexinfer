...
func TestNegotiateProtocolVersion(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			raw:  json.RawMessage(`{"protocolVersion":"2099-01-01"}`),
			want: mcp.ProtocolVersion20250618,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := negotiateProtocolVersion(tt.raw); got != tt.want {
				t.Fatalf("negotiateProtocolVersion() = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestHandleInitialize_UsesNegotiatedProtocolVersion(t *testing.T) {
	d := &Daemon{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := d.handleInitialize(context.Background(), msg)
			if err != nil {
			...
			if result.ProtocolVersion != tt.want {
				t.Fatalf("handleInitialize() protocol = %q, want %q", result.ProtocolVersion, tt.want)
			}
		})
	}
}
