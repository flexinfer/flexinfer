<script lang="ts">
  ...
  // Reads /api/mills/policy/proposals (slice 7.1 → 7.2 proxy) via the
  // shared MillsStore so proposals refresh at the standard 15s cadence.
  // Apply/Reject hit POST /api/mills/policy/proposals/{id}/{apply|reject}
  ...
  async function onApply(id: number): Promise<void> {
    ...
    try {
      await millsStore.applyPolicyProposal(id);
    } catch (e) {
  ...
  async function onReject(id: number): Promise<void> {
    try {
      await millsStore.rejectPolicyProposal(id);
    } catch (e) {
      // eslint-disable-next-line no-console
