<script>
  ...
  async function fetchSnapshot() {
    try {
      ...
      error = e.message ?? 'Failed to fetch orchestration status';
    } finally {
      loading = false;
    }
  }
  ...
  async function fetchPolicies() {
    try {
      const res = await fetch('/api/orchestration/policies');
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  async function runPreflight() {
    ...
    try {
      const res = await fetch('/api/orchestration/preflight', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
<div class="orchestration-panel">
  ...
  {#if loading}
    <div class="loading">Loading orchestration data...</div>
  {:else if error}
