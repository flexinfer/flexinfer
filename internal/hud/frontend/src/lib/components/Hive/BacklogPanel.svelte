<script lang="ts">
  import type { CostEstimate } from '../../stores/hive.svelte.ts';
  ...
  let previews = $derived(hiveStore.costPreviews);
  ...
  $effect(() => {
    ...
    fetchedIDs = next;
    void Promise.all(missing.map((id) => hiveStore.fetchCostPreview(id)));
  });
