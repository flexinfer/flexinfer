<script lang="ts">
  ...
  let scores = $derived(hiveStore.evalScores);
  let loading = $derived(hiveStore.loading && hiveStore.evalScores.length === 0);
...
<PanelShell
  ...
  emptyIcon={disabled ? '◯' : '□'}
  emptyMessage={disabled ? 'Hive operator not configured' : (error ? 'Failed to load eval scores' : 'No eval scores yet')}
  emptyHint={disabled ? 'Set LOOM_HIVE_OPERATOR_URL on the HUD to connect.' : (error ?? 'Loop A scores artifacts; Loop B scores merges; Loop C runs weekly.')}
>
  {#snippet header()}
    <div class="loop-row">
