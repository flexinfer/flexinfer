<script lang="ts">
  ...
  // loadDebate() that caches in hiveStore.debateByRun, so subsequent
  ...
  function toggle(runID: string): void {
    ...
    if (expanded[runID]) {
      void hiveStore.loadDebate(runID);
    }
  }
  ...
  <table class="hive-table">
    ...
    <tbody>
      {#each runs as r (r.ID)}
        ...
        {@const debate = hiveStore.debateByRun[r.ID]}
            ...
            <td colspan="6">
              ...
              {:else if debate.status === 'error'}
                <div class="debate-status error">
                  ...
                  <button type="button" on:click|stopPropagation={() => hiveStore.loadDebate(r.ID)}>retry</button>
                </div>
