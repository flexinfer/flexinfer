<script lang="ts">
  // HiveKPIRow renders the five canonical hive KPIs as a horizontal card
  // row for the Overview panel. Source data is the operator's
  // /api/hive/kpis snapshot; trend sparklines are computed from the
  // store's in-memory polling history. The row hides itself when the
  // hive operator is not configured so the Overview stays calm for users
  // who aren't running the hive.
  //
  ...
  let metrics = $derived(hiveStore.kpis?.metrics ?? null);
  ...
  let cards = $derived.by(() => {
    ...
    const cards: Array<{
    ...
    }> = [
      {
        ...
        trend: hiveStore.metricSeries('cost_per_merged_change_usd'),
        trendColor: trendColor(hiveStore.metricSeries('cost_per_merged_change_usd'), 'lower-better'),
      },
      {
        label: 'Slice→merge p50',
        value: fmtDuration(m.slice_to_merge_p50_seconds),
        trend: hiveStore.metricSeries('slice_to_merge_p50_seconds'),
        trendColor: trendColor(hiveStore.metricSeries('slice_to_merge_p50_seconds'), 'lower-better'),
      },
      {
        label: 'Gate pass rate',
        value: fmtPct(m.gate_pass_rate),
        trend: hiveStore.metricSeries('gate_pass_rate'),
        trendColor: trendColor(hiveStore.metricSeries('gate_pass_rate'), 'higher-better'),
      ...
      },
      {
        ...
        trend: hiveStore.metricSeries('auto_merge_rate'),
        trendColor: trendColor(hiveStore.metricSeries('auto_merge_rate'), 'neutral'),
      },
      {
        label: 'Regression rate',
        value: fmtPct(m.regression_rate),
        trend: hiveStore.metricSeries('regression_rate'),
        trendColor: trendColor(hiveStore.metricSeries('regression_rate'), 'lower-better'),
  ...
  function gotoHive() {
    router.navigate('hive');
  }
</script>

{#if !disabled}
  <section class="hive-kpi-section">
    <div class="hive-kpi-header">
      <span class="hive-kpi-label">Hive</span>
      <span class="hive-kpi-note">
        {#if metrics}
          Rolling 24h
        {:else}
