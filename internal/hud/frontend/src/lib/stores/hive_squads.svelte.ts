// Hive Squads store — per-squad rows + outcome stats from the
// loom-hive-operator, proxied through /api/hive/squads* by the HUD's
...
import type { Squad, SquadMemory } from './hive_squads_types.ts';
...
// SquadsListEntry is one row from GET /api/hive/squads.
export interface SquadsListEntry {
  squad: Squad;
  outcome_stats: SquadOutcomeStats;
}

// SquadDetail is the response from GET /api/hive/squads/{name}. The store
...
class HiveSquadsStore {
  ...
  async refresh(): Promise<void> {
    ...
    this.error = null;
    try {
      const list = await this.getJSON<SquadsListEntry[]>('/api/hive/squads');
      this.state = list ?? [];
  ...
  async fetchDetail(name: string): Promise<SquadDetail | null> {
    ...
    try {
      const detail = await this.getJSON<SquadDetail>(
        `/api/hive/squads/${encodeURIComponent(name)}`,
      );
      if (detail) {
...
export const hiveSquadsStore = new HiveSquadsStore();
