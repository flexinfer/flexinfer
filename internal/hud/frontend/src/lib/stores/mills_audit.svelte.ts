// Mills Audit store — adversarial-audit findings from the
// loom-mills-operator, proxied through /api/mills/audit/* by the HUD's
...
// AuditFinding mirrors the Go shape `pkg/mills/store.AuditFinding`. Field
...
class MillsAuditStore {
  ...
  async refresh(): Promise<void> {
    ...
    this.error = null;
    try {
      const list = await this.getJSON<AuditFinding[]>('/api/mills/audit/findings?limit=200');
      this.state = list ?? [];
...
export const millsAuditStore = new MillsAuditStore();
