// Hive Cross-Repo store — atomic-merge run inventory + abort surface
// from the loom-hive-operator, proxied through /api/hive/cross-repo/* by
// the HUD's domain/hive package. Polls the list endpoint at 15s by default
// so the cadence agrees with the rest of the Hive panels.
//
...
import {
  ...
  type CrossRepoRun,
  type CrossRepoState,
} from './hive_crossrepo_types.ts';

const ATOMICITY_WINDOW = 30;

class HiveCrossRepoStore {
  ...
  async refresh(): Promise<void> {
    ...
    this.error = null;
    try {
      const resp = await this.getJSON<CrossRepoListResponse>('/api/hive/cross-repo/runs');
      this.runs = resp?.runs ?? [];
  ...
  async fetchDetail(id: string): Promise<CrossRepoRun | null> {
    ...
    try {
      const detail = await this.getJSON<CrossRepoRun>(
        `/api/hive/cross-repo/runs/${encodeURIComponent(id)}`,
      );
      if (detail) {
  ...
  async abort(id: string, adminToken: string): Promise<CrossRepoAbortResponse> {
    ...
    const res = await globalThis.fetch(
      `/api/hive/cross-repo/runs/${encodeURIComponent(id)}/abort`,
      {
        method: 'POST',
...
export const hiveCrossRepoStore = new HiveCrossRepoStore();
