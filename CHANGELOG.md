...
- CI static-analysis jobs now target first-party packages (`./cmd`, `./internal`, `./pkg`) to avoid scanning `.go/pkg/mod`; `golangci-lint` timeout increased to 10m, `gosec` now runs with reduced concurrency, and `govulncheck` now runs in module-scan mode with `GOMEMLIMIT=6GiB` to reduce OOM failures on the k3s runner.
