...
- CI static-analysis jobs now target first-party packages (`./cmd`, `./internal`, `./pkg`) to avoid scanning `.go/pkg/mod`; `golangci-lint` timeout increased to 10m, `gosec` now runs with reduced concurrency, and `govulncheck` now uses `-scan=module` plus `GOMEMLIMIT=6GiB` to reduce OOM failures on the k3s runner.
