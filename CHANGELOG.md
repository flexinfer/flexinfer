...
### Changed
- CI now seeds Go module/build cache in a dedicated `prepare` stage (`pull-push`), and lint jobs skip eager `go mod download` prewarm while using shallow clones for `../../libs/*` replacements.
