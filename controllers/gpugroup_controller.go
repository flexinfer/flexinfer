...
// Helper to get queue depth from annotations
func getQueueDepth(annotations map[string]string, modelName string) int32 {
	if annotations == nil {
		return 0
	}
	key := AnnotationQueueDepthPrefix + modelName
	if val, ok := annotations[key]; ok {
		if depth, err := strconv.ParseInt(val, 10, 32); err == nil {
			return int32(depth)
		}
	}
	return 0
...
// Helper to parse queue since time from annotations
func getQueueSince(annotations map[string]string, modelName string) *time.Time {
	if annotations == nil {
		return nil
	}
	key := AnnotationQueueSincePrefix + modelName
	if val, ok := annotations[key]; ok {
...
// clearQueueAnnotations clears queue annotations for a model after it becomes active
func clearQueueAnnotations(annotations map[string]string, modelName string) {
	if annotations == nil {
		return
	}
	delete(annotations, AnnotationQueueDepthPrefix+modelName)
	delete(annotations, AnnotationQueueSincePrefix+modelName)
}

// isModelReady checks if a ModelDeployment is ready to serve requests
func isModelReady(md *aiv1alpha1.ModelDeployment) bool {
	if md == nil {
		return false
	}
	...
	for _, condition := range md.Status.Conditions {
		if condition.Type == aiv1alpha1.ConditionTypeReady {
...
// getMemberPriority gets the priority for a member from GPUGroup spec
func getMemberPriority(gpuGroup *aiv1alpha1.GPUGroup, modelName string) int32 {
	for _, member := range gpuGroup.Spec.Models {
		if member.Name == modelName {
...
// ensureModelCaches creates RAM-backed ModelCache resources if AutoCacheModels is enabled
func (r *GPUGroupReconciler) ensureModelCaches(ctx context.Context, gpuGroup *aiv1alpha1.GPUGroup, members map[string]*aiv1alpha1.ModelDeployment) error {
	if !gpuGroup.Spec.AutoCacheModels {
		return nil
	}
	...
	for _, member := range gpuGroup.Spec.Models {
		md, ok := members[member.Name]
		if !ok {
			continue
		}

		// Skip if ModelDeployment already has a cache reference
		if md.Spec.ModelCacheRef != nil {
			continue
		}

		cacheName := fmt.Sprintf("%s-ram-cache", member.Name)

		// Check if cache already exists
		existingCache := &aiv1alpha1.ModelCache{}
		err := r.Get(ctx, client.ObjectKey{Name: cacheName, Namespace: gpuGroup.Namespace}, existingCache)
		if err == nil {
			// Cache exists, skip
			continue
		}
		...
		// Determine model source from ModelDeployment
		source := getModelSource(md)
		if source == "" {
			log.Info("Cannot determine model source for auto-cache", "model", member.Name)
			continue
		}

		// Create RAM-backed ModelCache
		cache := &aiv1alpha1.ModelCache{
			ObjectMeta: metav1.ObjectMeta{
				Name:      cacheName,
				Namespace: gpuGroup.Namespace,
				Labels: map[string]string{
					"flexinfer.ai/gpu-group": gpuGroup.Name,
					"flexinfer.ai/model":     member.Name,
				},
			},
			Spec: aiv1alpha1.ModelCacheSpec{
				Source:          source,
				StorageStrategy: aiv1alpha1.StorageStrategyMemory,
				NodeSelector:    gpuGroup.Spec.NodeSelector,
			},
		}

		// Set owner reference
		if err := ctrl.SetControllerReference(gpuGroup, cache, r.Scheme); err != nil {
			return err
		}

		if err := r.Create(ctx, cache); err != nil {
			return fmt.Errorf("failed to create ModelCache %s: %w", cacheName, err)
		}

		log.Info("Created RAM cache for model", "cache", cacheName, "model", member.Name)
		r.Recorder.Eventf(gpuGroup, "Normal", "ModelCacheCreated",
			"Created RAM cache %s for model %s", cacheName, member.Name)
	}

	return nil
}

// getModelSource extracts the model source URI from a ModelDeployment
func getModelSource(md *aiv1alpha1.ModelDeployment) string {
	// For MLC-LLM models, construct HF source
	backend := strings.ToLower(md.Spec.Backend)
	if backend == "mlc-llm" || backend == "mlc" {
		if strings.Contains(md.Spec.Model, "/") {
			// Looks like a HuggingFace model ID
			return "HF://" + md.Spec.Model
		}
		// Try mlc-ai prefix
		return "HF://mlc-ai/" + md.Spec.Model
	}

	// For other backends, try HuggingFace if it looks like a model ID
	if strings.Contains(md.Spec.Model, "/") {
		return "huggingface://" + md.Spec.Model
	}
