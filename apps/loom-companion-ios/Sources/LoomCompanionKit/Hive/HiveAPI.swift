// Hive API client (Phase 7 slice 7.5).
//
// Thin wrapper over LoomAPIClientProtocol that exposes the two read
// endpoints the mobile Hive screen consumes: pipeline runs and the
// latest KPI snapshot. Both are proxied by the HUD's /api/hive/* tier
// (see internal/hud/domain/hive/hive.go); this file does NOT talk to
// the operator directly. When the operator is unconfigured the proxy
// returns 503 — the screen surfaces that as a "Hive disabled" empty
// state, so all errors thrown here just bubble up via the standard
// LoomAPIError.

import Foundation

/// Mirrors `pkg/hive/store.PipelineRun` (default Go JSON encoding uses
/// uppercase field names, which is why CodingKeys do, too).
public struct HivePipelineRun: Codable, Sendable, Identifiable, Hashable {
...
/// Mirrors `pkg/hive/store.KPISnapshot`. `metrics` is an open map so
...
public struct HiveKPISnapshot: Codable, Sendable, Hashable {
    public let id: Int64?
...
/// Read-only Hive client surface. ViewModels and previews depend on the
/// protocol so test fakes can short-circuit network calls.
public protocol HiveAPIProtocol: Sendable {
    func pipelineRuns() async throws -> [HivePipelineRun]
    func latestKPI(window: String) async throws -> HiveKPISnapshot?
}

/// Concrete client backed by the existing `LoomAPIClientProtocol`. The
/// HUD's hive proxy returns 404 when no KPI snapshot exists yet — that
/// surfaces as `LoomAPIError.notFound` from the underlying client; the
/// Hive screen treats that as "no data yet" rather than an error.
public struct HiveAPI: HiveAPIProtocol, Sendable {
    ...
    public func pipelineRuns() async throws -> [HivePipelineRun] {
        try await client.request(.hivePipelineRuns)
    }

    public func latestKPI(window: String = "1d") async throws -> HiveKPISnapshot? {
        do {
            let snap: HiveKPISnapshot = try await client.request(.hiveKPIs(window: window))
            return snap
        } catch let LoomAPIError.apiError(code, _, _) where code == .notFound || code == .notConfigured {
            // No snapshot yet, or LOOM_HIVE_OPERATOR_URL unset → render
