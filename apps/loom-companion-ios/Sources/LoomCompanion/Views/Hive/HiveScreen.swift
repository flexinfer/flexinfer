...
// from this screen. The HiveAPI is injected so previews + tests can
// short-circuit network calls.

import LoomCompanionKit
import SwiftUI

public struct HiveScreen: View {
    @State private var pipelineRuns: [HivePipelineRun] = []
    @State private var kpi: HiveKPISnapshot?
    @State private var loading = true
    @State private var loadError: String?

    private let api: HiveAPIProtocol?

    /// Initializer accepting an optional concrete API. When the user
    /// hasn't paired with a HUD yet, `connectionVM.buildAPIClient()`
    /// returns nil; the screen renders a "connect to view Hive" empty
    /// state in that case.
    public init(apiClient: APIClient?) {
        self.api = apiClient.map(HiveAPI.init(client:))
    }

    /// Test-only initializer that accepts a fake protocol implementation.
    public init(api: HiveAPIProtocol?) {
        self.api = api
    }
    ...
    private var pipelinesList: some View {
        Group {
            if loading && pipelineRuns.isEmpty {
                ...
                emptyState(icon: "checkmark.circle", message: "No pipelines running", hint: "Idle hive — new work will land here.")
            } else {
    ...
    private var inFlight: [HivePipelineRun] {
        pipelineRuns.filter { !["done", "merged", "escalated", "failed"].contains($0.state) }
    }

    private func pipelineRow(_ run: HivePipelineRun) -> some View {
    ...
    private func depthPill(_ run: HivePipelineRun) -> some View {
    ...
    private func reload() async {
        guard let api else {
            loading = false
            loadError = "Pair with a HUD to view Hive."
            return
        }
        loading = true
        loadError = nil
...
#Preview {
    NavigationStack {
        HiveScreen(api: HiveScreenPreviewAPI())
    }
}

private struct HiveScreenPreviewAPI: HiveAPIProtocol, Sendable {
    func pipelineRuns() async throws -> [HivePipelineRun] {
        [
            HivePipelineRun(id: "PIPE-001", backlogID: "BACK-A", template: "hive-default",
                            state: "implementing", attempts: 1, depth: 0),
            HivePipelineRun(id: "PIPE-001-S1", backlogID: "BACK-A-CHILD", template: "hive-default",
                            state: "testing", attempts: 1, parentRunID: "PIPE-001", depth: 1),
        ]
    }
    func latestKPI(window: String) async throws -> HiveKPISnapshot? {
        HiveKPISnapshot(metrics: [
