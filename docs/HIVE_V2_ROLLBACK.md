# Loom Hive v2 — Rollback Playbook
...
| Operator boot loop / DB integrity check fails | [Restore DB from MinIO backup](HIVE_RUNBOOK.md#recover-from-a-corrupted-db) |
...
yq -i '.policy.squads.enabled = false' k3s/hive/configmap-policy.yaml
git commit -am "ops(hive): disable squads (incident YYYY-MM-DD)"
git push
flux reconcile kustomization apps -n flux-system
# Confirm hot-reload:
kubectl logs -n loom-hive deploy/loom-hive-operator --tail=20 | grep "policy reloaded"
```

After disable, in-flight squad-routed runs continue to terminal state — disable affects routing decisions for *new* backlog items only. To kill in-flight runs, force-escalate them per [HIVE_RUNBOOK.md → Force-escalate a stuck pipeline run](HIVE_RUNBOOK.md#force-escalate-a-stuck-pipeline-run).
...
yq -i '.policy.audit.advisory_only = true' k3s/hive/configmap-policy.yaml

# Heavier: stop running audits at all:
yq -i '.policy.audit.enabled = false' k3s/hive/configmap-policy.yaml
```
...
yq -i '.policy.council.debate.enabled.cron = false' k3s/hive/configmap-policy.yaml

# Or fully:
yq -i '.policy.council.debate.enabled.cron = false' k3s/hive/configmap-policy.yaml
yq -i '.policy.council.debate.enabled.roadmap = false' k3s/hive/configmap-policy.yaml
yq -i '.policy.council.debate.enabled.incident = false' k3s/hive/configmap-policy.yaml
```
...
yq -i '.policy.cross_repo.enabled = false' k3s/hive/configmap-policy.yaml
git commit -am "ops(hive): disable cross_repo (incident YYYY-MM-DD)"
git push && flux reconcile kustomization apps -n flux-system

# 2. Find in-flight cross-repo runs:
curl -sf "$LOOM_HIVE_OPERATOR_URL/api/hive/cross-repo/runs?state=running" | jq

# 3. Abort each one (admin token required):
curl -X POST -H "Authorization: Bearer $LOOM_HIVE_ADMIN_TOKEN" \
  "$LOOM_HIVE_OPERATOR_URL/api/hive/cross-repo/runs/<id>/abort"

# 4. The integrator's revert path runs automatically when abort is called
#    on a run that already merged one side; verify:
curl -sf "$LOOM_HIVE_OPERATOR_URL/api/hive/cross-repo/runs/<id>" | jq '.state'
# expect: "reverted" within ~60s, or "revert_failed" requiring manual cleanup.
```

If `revert_failed`: the per-MR rollback plan is in `HIVE_RUNBOOK.md → Force-revert a cross-repo merge`. Each side is a regular MR — `git revert` + open a follow-up MR to drop the change cleanly.
...
```bash
yq -i '.policy.adaptive_policy.enabled = false' k3s/hive/configmap-policy.yaml
```
...
> **Note**: The DAO exposes `Revert(id)` (`pkg/hive/store/dao_policy_proposal.go`) but no REST endpoint is wired in v2.0. A future slice may surface it on the HUD card; for now, the manual flow below is canonical.
...
ID=42
curl -sf "$LOOM_HIVE_OPERATOR_URL/api/hive/policy/proposals/$ID" | jq -r .Diff
...
git commit -am "ops(hive): revert policy proposal $ID"
git push && flux reconcile kustomization apps -n flux-system
...
curl -X POST -H "Authorization: Bearer $LOOM_HIVE_ADMIN_TOKEN" \
  "$LOOM_HIVE_OPERATOR_URL/api/hive/policy/proposals/$ID/reject"
```
...
kubectl logs -n loom-hive deploy/loom-hive-operator --tail=50 | grep -i "policy reloaded"

# 2. Live policy matches the gitops repo:
curl -sf "$LOOM_HIVE_OPERATOR_URL/api/hive/policy" | jq '.squads.enabled'
# expect: false
...
#    stop landing:
curl -sf "$LOOM_HIVE_OPERATOR_URL/api/hive/squads" | jq '.[].LastOutcomeAt' | sort -u
```
...
- [docs/HIVE.md](HIVE.md) — v1 architecture + v2 forward references
- [docs/HIVE_RUNBOOK.md](HIVE_RUNBOOK.md) — operator runbook (DB restore, force-escalate, audit a merged change)
- `.loom/93-product-spec-hive-v2-hierarchical-swarm-2026-05-02.md` §"Failure modes"
- `.loom/94-implementation-plan-hive-v2-hierarchical-swarm-2026-05-02.md` Phase 8 slices 8.1–8.3
- `pkg/hive/policy.go` — `enabled` field on every v2 policy struct
- `pkg/hive/store/dao_policy_proposal.go` — `Apply` / `Reject` / `Revert` DAO methods
- `cmd/loom-hive-operator/handlers_policy_proposals.go` — REST endpoints (Phase 7 slice 7.2)
