...
| Moonlight video connects but controller/keyboard/mouse input does nothing | Sway must run with `WLR_BACKENDS=headless,libinput`, `gamer` must be in the host gids that own `/dev/input/event*`, and the gaming profile must mount `/dev/input`, `/dev/uinput`, and `/run/udev/data`. Verify with `swaymsg -t get_inputs` inside the runtime pod; it should not be `[]` after a client connects. |
