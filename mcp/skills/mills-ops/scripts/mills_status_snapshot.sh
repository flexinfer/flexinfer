...
#
# Required env:
#   LOOM_MILLS_OPERATOR_URL   e.g. http://mills.loom.lan or cluster service URL
...
OPERATOR_URL="${LOOM_MILLS_OPERATOR_URL:-}"
TOKEN="${LOOM_ADMIN_TOKEN:-}"

if [[ -z "${OPERATOR_URL}" ]]; then
  echo "error: LOOM_MILLS_OPERATOR_URL is not set" >&2
  exit 2
fi
if [[ -z "${TOKEN}" ]]; then
...
if command -v kubectl >/dev/null 2>&1; then
  ...
  printf '```\n'
  kubectl logs -n "${NS}" deploy/loom-mills-operator --tail=30 2>&1 || true
  printf '```\n\n'
else
...
printf '```json\n'
curl_op '/api/mills/pipeline/runs?limit=50' | (command -v jq >/dev/null && jq . || cat) || \
  printf '%s\n' "(unreachable)"
printf '\n```\n\n'
...
for subj in council_run pipeline_run cross_run; do
  printf '### %s\n\n' "${subj}"
  printf '```json\n'
  curl_op "/api/mills/eval?subject_kind=${subj}&limit=20" | \
...
printf '```\n'
curl -sf "${OPERATOR_URL}/metrics" 2>/dev/null | grep -E '^loom_mills_(pipeline_cost|merge_to_main|gate_decisions|regression|budget)' || \
  printf '%s\n' "(metrics unreachable)"
printf '\n```\n'
