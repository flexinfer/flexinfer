...
5. **Verify.** Operator hot-reloads policy via fsnotify within seconds.
   ...
   v2 flips) `loom_mills_regression_count_total` over the next 24h.
...
# score via /api/mills/eval?subject_kind=council_run&subject_id=<id>
```
...
Watch `loom_mills_regression_count_total` after each flip; pause via
