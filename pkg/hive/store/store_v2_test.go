...
// Hive v2 — Hierarchical Swarm DAO round-trip tests. These exercise the new
...
func TestSquadOutcomes_SuccessRate(t *testing.T) {
	...
	for i, kind := range []SquadOutcomeKind{
		...
		runID := nextRunID(t, i)
		if err := seedPipelineRun(ctx, st, runID, "HIVE-2026-05-02-001", i+1); err != nil {
...
func TestCrossRepo_Lifecycle(t *testing.T) {
	...
	if err := st.Backlog.Put(ctx, &BacklogItem{
		ID: "HIVE-2026-05-02-001", Title: "Cross-repo test", State: BacklogQueued,
	...
	r := &CrossRepoRun{
		...
		BacklogItemID: "HIVE-2026-05-02-001",
	...
	// ListByBacklog.
	byBacklog, err := st.CrossRepo.ListByBacklog(ctx, "HIVE-2026-05-02-001")
	if err != nil {
...
func TestPipelineRun_RecursionFields(t *testing.T) {
	...
	if err := st.Backlog.Put(ctx, &BacklogItem{
		ID: "HIVE-2026-05-02-001", Title: "recursion test", State: BacklogQueued,
		Priority: P2, CreatedBy: "council", CouncilRunID: &council,
	}); err != nil {
		t.Fatalf("seed backlog: %v", err)
	}
	...
	// doesn't collide. The dispatcher in pkg/hive/pipeline/recursion
	...
	if err := st.Pipeline.PutRun(ctx, &PipelineRun{
		ID: "PIPE-BAD", BacklogID: "HIVE-2026-05-02-001",
		Template: "hive-default-pipeline", State: PipelineQueued,
