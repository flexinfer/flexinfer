...
func newTestStore(t *testing.T) *Store {
	t.Helper()
	dir := t.TempDir()
	st, err := Open(context.Background(), Options{Path: filepath.Join(dir, "hive.db")})
	if err != nil {
...
func TestBacklog_RoundTrip(t *testing.T) {
	...
	item := &BacklogItem{
		...
		Dependencies: []string{"HIVE-2026-04-25-000"},
	...
	queueOnly := &BacklogItem{
		ID: "HIVE-2026-04-25-002", Title: "second", State: BacklogQueued,
...
func TestCouncil_RoundTrip(t *testing.T) {
	...
	run := &CouncilRun{
		...
		BacklogDeltas: BacklogDeltas{Created: []string{"HIVE-2026-04-25-001"}},
...
func TestPipeline_RoundTrip(t *testing.T) {
	...
	parent := &BacklogItem{
		ID: "HIVE-X", Title: "x", State: BacklogQueued, Priority: P2, CreatedBy: "test",
	}
...
func TestRoadmap_UpsertAndStale(t *testing.T) {
	...
	a := &RoadmapIntent{
		...
		Priority:             1,
		Summary:              "Loom Hive — council + pipeline",
