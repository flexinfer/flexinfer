...
// correlating an Alertmanager fire with a recent hive merge. Spec
...
// RegressionGate correlates Alertmanager firings with recent hive
...
//  2. Lists hive-merged pipeline runs whose EndedAt falls inside
...
func (g *RegressionGate) OnAlert(ctx context.Context, alert AlertEvent) (*RegressionResult, error) {
	...
	for _, run := range merged {
		...
		hive.RegressionCountTotal.WithLabelValues(res.AlertName, res.Severity).Inc()
	...
	if len(res.Correlated) > 0 && g.autoRevertEnabled() {
		...
		hive.RegressionAutoRevertPendingTotal.Add(float64(len(res.Correlated)))
	}
	return res, nil
}
