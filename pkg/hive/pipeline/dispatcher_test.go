...
func sampleJobContext(stageID string, opts ...func(*JobContext)) JobContext {
	...
	jc := JobContext{
		...
		Budget: item.Budget,
		Env:    BuildHiveEnv(run, item, stage),
	}
	for _, o := range opts {
		o(&jc)
	}
...
func TestBuildHiveEnv_AlwaysIncludesIDs(t *testing.T) {
	jc := sampleJobContext("implement")
	env := jc.Env
	want := map[string]string{
		"LOOM_HIVE_RUN_ID":       "PIPE-X-1",
		"LOOM_HIVE_BACKLOG_ID":   "BL-X",
		"LOOM_HIVE_STAGE":        "implement",
		"LOOM_PARENT_SESSION_ID": "claude-code-session-9",
		"LOOM_HIVE_WORKTREE":     "/tmp/wt",
	}
	for k, v := range want {
		if env[k] != v {
...
func TestBuildHiveEnv_OmitsOptionalWhenAbsent(t *testing.T) {
	jc := sampleJobContext("plan_slice", func(jc *JobContext) {
		jc.Run.ParentSessionID = ""
		jc.Run.WorktreePath = ""
		jc.Env = BuildHiveEnv(jc.Run, jc.Item, jc.Stage)
	})
	if _, ok := jc.Env["LOOM_PARENT_SESSION_ID"]; ok {
		t.Errorf("LOOM_PARENT_SESSION_ID should be omitted when empty")
	}
	if _, ok := jc.Env["LOOM_HIVE_WORKTREE"]; ok {
		t.Errorf("LOOM_HIVE_WORKTREE should be omitted when empty")
	}
}
...
func TestSpawnWorker_PropagatesBudgetEnvAndPrompt(t *testing.T) {
	...
	if got.Env["LOOM_HIVE_RUN_ID"] != "PIPE-X-1" {
