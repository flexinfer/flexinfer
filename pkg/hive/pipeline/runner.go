// Package pipeline drives a backlog item through the hive-default-pipeline
// DAG defined in cmd/mcp-mentatlab/templates/hive-default-pipeline.yaml.
//
...
type Stage struct {
	// ID matches the node id in hive-default-pipeline.yaml.
	ID string
...
// DefaultStages mirrors hive-default-pipeline.yaml. Order is significant.
//
...
func New(s *store.Store, gr *gates.Registry, d WorkerDispatcher, pm *hive.PolicyManager) *Runner {
	return &Runner{
		Store:      s,
		Gates:      gr,
		Dispatcher: d,
...
// Start satisfies hive.PipelineStarter. It validates inputs, kicks off
...
// produced it.
func (r *Runner) gateInputFor(stage Stage, item *store.BacklogItem, policy *hive.Policy, prior map[string]StageOutput) gates.StageInput {
...
func (r *Runner) policy() *hive.Policy {
	if r.Policy == nil {
		return hive.Default()
	}
