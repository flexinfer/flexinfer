...
type JobContext struct {
	...
	Budget store.Budget
	// Env carries the LOOM_HIVE_* variables every worker propagates to
...
// BuildHiveEnv returns the canonical LOOM_HIVE_* env-var bundle every
...
func BuildHiveEnv(run *store.PipelineRun, item *store.BacklogItem, stage Stage) map[string]string {
	env := map[string]string{
		"LOOM_HIVE_RUN_ID":     run.ID,
		"LOOM_HIVE_BACKLOG_ID": item.ID,
		"LOOM_HIVE_STAGE":      stage.ID,
	}
	if run.ParentSessionID != "" {
		env["LOOM_PARENT_SESSION_ID"] = run.ParentSessionID
	}
	if run.WorktreePath != "" {
		env["LOOM_HIVE_WORKTREE"] = run.WorktreePath
	}
	return env
}
