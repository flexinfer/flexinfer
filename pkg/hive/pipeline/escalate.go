...
type HandoffRequest struct {
	From        string         // "loom-hive-operator"
...
func (e *Escalator) Handle(ctx context.Context, run *store.PipelineRun, item *store.BacklogItem, reason string) error {
	...
	issueURL := ""
	if e.Issue != nil {
		title := fmt.Sprintf("[hive] %s escalated: %s", run.ID, truncate(reason, 80))
		req := IssueRequest{
			BacklogID:   item.ID,
			Title:       title,
			Description: e.renderIssueBody(rec),
			Labels:      []string{"hive-escalation", "kind/incident", "priority/" + string(item.Priority)},
		}
