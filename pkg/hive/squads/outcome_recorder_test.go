...
// shape the reconciler writes at routing time. Mirrors the hive package's
...
func TestOutcomeRecorder_HappyPath(t *testing.T) {
	...
	run, item := seedRunForRecorder(t, st, "PIPE-OK-1", "HIVE-OK-1", 1)
...
func TestOutcomeRecorder_NoEventIsNoOp(t *testing.T) {
	st := newTestStore(t)
	run, item := seedRunForRecorder(t, st, "PIPE-NOEVT-1", "HIVE-NOEVT-1", 1)
...
func TestOutcomeRecorder_FallbackSquadIsNoOp(t *testing.T) {
	st := newTestStore(t)
	run, item := seedRunForRecorder(t, st, "PIPE-FB-1", "HIVE-FB-1", 1)
	seedRoutedEvent(t, st, run.ID, map[string]any{
...
func TestOutcomeRecorder_DoubleFireIsIdempotent(t *testing.T) {
	...
	run, item := seedRunForRecorder(t, st, "PIPE-DBL-1", "HIVE-DBL-1", 1)
	seedRoutedEvent(t, st, run.ID, map[string]any{
...
func TestOutcomeRecorder_MarkRegressed(t *testing.T) {
	...
	run, item := seedRunForRecorder(t, st, "PIPE-REG-1", "HIVE-REG-1", 1)
	seedRoutedEvent(t, st, run.ID, map[string]any{
		"squad_name": "hud-frontend", "path_class": "internal/hud/frontend/**",
	})
