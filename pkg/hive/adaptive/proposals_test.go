...
func TestProposalsBuilder_Tighten(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		seedEval(t, st, store.EvalSubjectPipelineRun, runID(i), hiveeval.PipelineOutcomeRubric, 0.5, now.Add(-3*24*time.Hour))
	}
	b := &ProposalsBuilder{
		Store:               st,
		DocsRoot:            t.TempDir(),
...
func TestProposalsBuilder_RotateEnsemble(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		seedEval(t, st, store.EvalSubjectCouncilRun, id, hiveeval.CouncilROIRubric, 0.9, now.Add(-5*24*time.Hour))
	}
	...
	for i := 0; i < 4; i++ {
		...
		seedEval(t, st, store.EvalSubjectCouncilRun, id, hiveeval.CouncilROIRubric, 0.4, now.Add(-5*24*time.Hour))
	}
...
// .loom/hive/policy_proposals/<date>.md under the configured DocsRoot, and
