...
func TestWorktreeAllocator_Allocate_HappyPath(t *testing.T) {
	...
	a := NewWorktreeAllocator(hub, "loom-hive-operator", "session-op-1", "/workspaces/loom-core")
	got, err := a.Allocate(context.Background(), pipeline.WorktreeRequest{
		BacklogID: "BL-X",
		SliceName: "alpha",
		Purpose:   "hive parallel slice alpha",
	})
	if err != nil {
	...
	if params.Arguments["agent_id"] != "loom-hive-operator" {
