...
func sampleSpawnReq() pipeline.SpawnRequest {
	return pipeline.SpawnRequest{
		...
		BacklogID:       "BL-X",
		Project:         "loom-core",
		Branch:          "hive/BL-X/plan_slice",
		BaseBranch:      "main",
		Namespace:       "loom-hive",
		Env:             map[string]string{"LOOM_HIVE_RUN_ID": "PIPE-X-1"},
	}
}
...
func TestRun_PostsCorrectRequestAndAuth(t *testing.T) {
	...
	if body.Branch != "hive/BL-X/plan_slice" {
	...
	if body.Metadata["loom_hive_stage"] != "plan_slice" {
		t.Errorf("metadata.loom_hive_stage missing: %v", body.Metadata)
	}
}
