...
// openairesponses.ResponsesClient interface.
func NewFlexInferResponsesClient(client *flexinfer.Client, logger *slog.Logger) *FlexInferResponsesClient {
	return &FlexInferResponsesClient{
		client: client,
		logger: logger.With("component", "orchestra-responses-client"),
	}
}
...
func (c *FlexInferResponsesClient) Create(ctx context.Context, req openairesponses.TurnRequest) (openairesponses.TurnResponse, error) {
	...
	// Apply Qwen3 /no_think prefix to suppress reasoning tokens.
	if len(chatReq.Messages) > 0 && strings.Contains(strings.ToLower(req.Model), "qwen3") {
		for i, msg := range chatReq.Messages {
			if msg.Role == "user" {
				chatReq.Messages[i].Content = "/no_think\n" + msg.Content
				break
			}
		}
	}
...
func (c *FlexInferResponsesClient) doRequest(ctx context.Context, body []byte) (*chatCompletionResponseWithTools, error) {
	...
	err := c.client.Breaker().Execute(func() error {
		...
		if err != nil {
			c.logger.Warn("orchestra request failed", "latency", latency, "error", err)
		...
		if resp.StatusCode != http.StatusOK {
			...
			c.logger.Warn("orchestra non-200", "status", resp.StatusCode, "body", string(respBody))
		...
		c.logger.Debug("orchestra completion",
			"model", result.Model,
			"latency", latency,
