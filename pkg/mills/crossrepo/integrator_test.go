...
// staticPolicy returns a fixed *mills.Policy snapshot for the integrator.
type staticPolicy struct{ p *mills.Policy }

func (s *staticPolicy) Snapshot() *mills.Policy { return s.p }
...
func integratorWithFakes(t *testing.T, perRepoMinutes int) (*Integrator, *fakeGitLab, *virtualClock) {
	...
	pol := &mills.Policy{
		Version: 2,
		CrossRepo: mills.CrossRepoPolicy{
			Enabled:               true,
