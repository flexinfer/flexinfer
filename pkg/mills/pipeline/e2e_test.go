...
// reconciler → pipeline.Runner (via mills.PipelineStarter contract) →
...
func TestPipeline_E2E_QueuedItemMergesWithEvalRow(t *testing.T) {
	...
	rec := mills.NewReconciler(st, pm, budget, runner)
...
// integration honors mills.Budget so a budget-exhausted item does not
...
// starterFn adapts a function into mills.PipelineStarter.
