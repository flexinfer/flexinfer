...
//
// Mills v2 lets a pipeline worker fan out into sub-runs (e.g. a refactor
...
//
// This file owns the *guards*; pkg/mills/store DAO owns the persistence
...
// in pkg/mills (see budget.go's tierLimits + RecursionPolicy doc).
const (
...
func (g *SubrunGuard) SubrunCreate(ctx context.Context, req SubrunRequest) (string, error) {
	...
	// subrun-pickup loop in pkg/mills/reconciler.go's Tick is the
	...
	mills.PipelineRecursionDepthHistogram.Observe(float64(newDepth))
	return newID, nil
}
