...
func TestRegressionGate_CorrelatesRecentMerge(t *testing.T) {
	...
	env.seedMergedRun("MILLS-A", 5*time.Minute)  // inside window
	env.seedMergedRun("MILLS-B", 10*time.Minute) // inside window
	env.seedMergedRun("MILLS-C", 45*time.Minute) // OUTSIDE window — must skip
...
func TestRegressionGate_ResolvedAlertSkipped(t *testing.T) {
	env := newRegressionEnv(t, false)
	env.seedMergedRun("MILLS-R", 1*time.Minute)
...
func TestRegressionGate_AutoRevertOptIn(t *testing.T) {
	...
	env.seedMergedRun("MILLS-X", 5*time.Minute)
	env.seedMergedRun("MILLS-Y", 10*time.Minute)
...
func TestRegressionGate_AppendsAuditEvent(t *testing.T) {
	env := newRegressionEnv(t, false)
	env.seedMergedRun("MILLS-AUD", 5*time.Minute)
