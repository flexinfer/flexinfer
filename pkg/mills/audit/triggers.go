...
// /api/mills/audit/run endpoint to test alternative pools).
...
// policy.audit.enabled flag without importing pkg/mills (which would
// invert the existing one-way import boundary). Production wires this
// to a closure over mills.PolicyManager.Current().AuditEnabled so the
...
type Triggers struct {
	...
	// Wired to the live mills.PolicyManager so hot-reload flips take
...
// OnArtifactsCommitted is the callback shape pkg/mills/runner.Runner
