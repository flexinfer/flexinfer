// Package adaptive holds the Sunday adaptive-policy job (Mills v2 Phase 7).
//
...
// digest at .loom/mills/policy_proposals/<date>.md.
//
// v2.0 proposals are advisory — a human (or the v2.1 auto-applier) reads the
// markdown, picks one, and runs `loom mills policy apply`. The rules below are
...
const (
	...
	MarkdownDir = ".loom/mills/policy_proposals"
...
func (b *ProposalsBuilder) evalRotate(in Inputs) []*store.PolicyProposal {
	...
	for _, sc := range in.EvalScores {
		...
		if sc.Rubric != millseval.CouncilROIRubric {
			continue
		}
...
func pipelineOutcomeMedian(scores []*store.EvalScore) (float64, int) {
	...
	for _, sc := range scores {
		...
		if sc.Rubric != millseval.PipelineOutcomeRubric {
			continue
		}
