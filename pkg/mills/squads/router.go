...
// policy.squads.enabled flag without importing pkg/mills (which would
// invert the existing one-way import boundary documented in
// pkg/mills/reconciler.go's SquadRouter comment). Production wires this
// to a closure over mills.PolicyManager.Current().SquadsEnabled so the
