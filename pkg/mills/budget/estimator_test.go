...
func newEstimatorEnv(t *testing.T) *estimatorEnv {
	...
	env := &estimatorEnv{
		t:      t,
		store:  st,
		policy: mills.Default(),
	}
	env.est = &Estimator{
		Store:      st,
		PolicyFunc: func() *mills.Policy { return env.policy },
	}
	return env
}
...
func (e *estimatorEnv) seedOutcomes(squad, pathClass string, n int, minCost, maxCost float64) {
	...
	itemID := fmt.Sprintf("MILLS-FIXTURE-%s", squad)
...
func TestEstimator_LowConfidence_NoHistory(t *testing.T) {
	...
	env.seedBacklog("MILLS-NOHIST", nil)

	got, err := env.est.Preview(context.Background(), "MILLS-NOHIST")
	if err != nil {
...
func TestEstimator_HighConfidence_KnownPathClass(t *testing.T) {
	...
	env.seedBacklog("MILLS-HIST", []string{"path_class:go-svc/internal/**"})

	got, err := env.est.Preview(context.Background(), "MILLS-HIST")
	if err != nil {
		t.Fatalf("preview: %v", err)
	}
...
func TestEstimator_RespectsEnsembleCap(t *testing.T) {
	...
	env.seedBacklog("MILLS-CAP", []string{"path_class:expensive/**"})

	got, err := env.est.Preview(context.Background(), "MILLS-CAP")
	if err != nil {
		t.Fatalf("preview: %v", err)
	}
...
func TestEstimator_RecursionOverhead_AddedWhenEnabled(t *testing.T) {
	...
	env.seedBacklog("MILLS-REC", []string{"path_class:rec/**"})

	// Recursion off → overhead zero.
	env.policy.Recursion.Enabled = false
	off, err := env.est.Preview(context.Background(), "MILLS-REC")
	if err != nil {
	...
	on, err := env.est.Preview(context.Background(), "MILLS-REC")
	if err != nil {
...
func TestEstimator_SidecarSliceCount(t *testing.T) {
	...
	env.seedBacklog("MILLS-SC", []string{
		"path_class:sc/**",
		"sidecar:docs",
		"sidecar:tests",
	})

	got, err := env.est.Preview(context.Background(), "MILLS-SC")
	if err != nil {
		t.Fatalf("preview: %v", err)
	}
...
func TestEstimator_UnknownBacklog_ReturnsErrNotFound(t *testing.T) {
	env := newEstimatorEnv(t)
	_, err := env.est.Preview(context.Background(), "MILLS-MISSING")
	if err == nil {
...
func TestEstimator_NotConfigured_ReturnsError(t *testing.T) {
	...
	e2 := &Estimator{Store: nil, PolicyFunc: func() *mills.Policy { return nil }}
