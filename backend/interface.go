...
func ROCmEnvVars(arch string) []corev1.EnvVar {
	...
	case strings.HasPrefix(arch, "gfx906"):
		// Vega20 (Radeon VII) is natively detected as gfx906 by the serving
		// runtime. Do not force HSA_OVERRIDE_GFX_VERSION: the certified llama.cpp
		// path ran on the real gfx906 target and an override changes the tested
		// runtime contract. Disable SDMA for stability on Vega20.
